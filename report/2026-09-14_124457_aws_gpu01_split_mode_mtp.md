...
> **【2026-09-14 14:34 JST 追記・訂正】本レポートの第 1 節「`--split-mode tensor` + MTP は初期化がハングする」は誤りである。**
> 後続の[原因調査レポート](./2026-09-14_142115_aws_gpu01_tensor_split_nccl_hang.md)で、
> ハングは **MTP とは無関係**で、`--split-mode tensor` 単独が**起動時に確率的に固まる**現象であることが判明した
> （MTP 無効でも同じ頻度で再現。NCCL 経路 1/15 成功に対し `GGML_CUDA_ALLREDUCE=butterfly` / `internal` では 15/15 成功、Fisher p = 9.6e-06）。
> `GGML_CUDA_ALLREDUCE=butterfly` を付ければ **tensor 分割と MTP は併用できる**（実推論で確認済み）。
> また副次発見 6 の「sudo が無いので gdb を見送った」も、実際には本機は NOPASSWD sudo が通るため、
> 技術的制約ではなく運用ポリシー上の判断だった。
>
> **【2026-09-16 11:05 JST 追記】欠測だった tensor 腕を[3 腕再測定レポート](./2026-09-16_103619_aws_gpu01_split_mode_mtp_3arm.md)で埋めた。**
> （なお上の `butterfly` は不正値で、正しくは `none` である。挙動は同一。）
> tensor + MTP は ctx=131072 でも問題なく動き、decode は深さ 128,793 tok で **25.78 t/s（初段比 105%）**。
> 本レポート第 2 節の「MTP 有効の layer が浅い深度で tensor を上回る」という交差は、**tensor 腕が入ると全深度で消滅する**。
> また第 5 節の「MTP は prefill を確実に遅くする」は **layer 腕については正しいが tensor 腕には当てはまらない**
> （`GGML_CUDA_ALLREDUCE` を揃えて比べると MTP は tensor の prefill を +29〜+32% 改善する）。
> 第 4 節の結論「MTP は tensor の代替にならない」は **3 腕そろえた再測定でも維持された**。
>
> **以下の本文は当時の記述をそのまま残す。**
...
> **【訂正】この節の結論（MTP との組み合わせ固有）は誤り。真因は `--split-mode tensor` 単独の
> NCCL AllReduce 起動時ハングで、MTP は無関係。詳細は[原因調査レポート](./2026-09-14_142115_aws_gpu01_tensor_split_nccl_hang.md)。**
...
6. **【訂正: 半分誤り】aws-gpu01 では `ptrace_scope` によりユーザ権限で gdb アタッチできない**（`ptrace_scope = 1` は事実だが、**本機は NOPASSWD sudo が通る**ので `sudo gdb` で取れる。見送ったのは運用ポリシー上の判断だった）。ハングの詳細解析には sudo が要るが、本機は sudo をユーザに依頼する運用のため今回は見送った。
...
- [tensor 分割と MTP は両立するが回避策が prefill を 7 割奪う](./2026-09-16_103619_aws_gpu01_split_mode_mtp_3arm.md) — 本レポートで欠測だった tensor 腕を埋めた 3 腕再測定

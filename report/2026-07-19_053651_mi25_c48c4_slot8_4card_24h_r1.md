...
副次的に、以前のレポートで「c48c4 個体を SLOT8 に移した後、単独可視化構成では prompt 処理速度が半減する」という原因未解明の観察が記録されていたが、今回の 4 枚同時運用ではこの半減現象は再現せず、むしろ元の SA 期の速度を上回る回復が見られた。つまり **prompt 半減は単独可視化構成に固有の現象** であることが分かった。さらに、単位時間あたりに完了できる推論リクエスト数も従来の SA 構成を上回っており、4 枚並列運用は VRAM 容量だけでなく実運用スループットの面でも優位である。GPU の温度・電力・PCIe いずれの物理指標も、問題個体を含めた 4 枚とも他の健全個体と同水準で推移した。

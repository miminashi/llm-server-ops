...
**副次発見の要点**:
- **制御ホスト側でブレーカー電源断 (2026-07-18 18:52-23:58 JST) — GPU サーバ (別拠点) は完全健全**: mi25 は uptime 継続、llama-server は孤児 request (task 758927) を推論し続け 5h 生存、dmesg fault / GPU reset / boot event 0 件。復旧後 Session2 として残 25 trial を継続 (23:58 → 05:36 JST、24h 制限を暫定緩和)。制御ホスト電源断は **統計解析には無影響** (trial_done event 単位で集計、部分実行 trial 75 は自動除外)
- **per-GPU power は 4 枚とも同水準 (mean 16.6-19.8W / p95 44-51W / cap 160W)** で分散、multi-GPU 分散負荷特有の per-card 高負荷スパイクは観測なし。max 172W は BACO-free 前提の PCIe train 期 or `--n-predict 32768` 完走時のバースト時に短時間出現、cap 160W は瞬時値には効かず p95 で見れば全 4 枚 51W 以下で余裕
...
- **eval_tps mean=12.85 t/s / p50=12.90 t/s** は SLOT8 SA (16.9 t/s) より約 24% 低下、4 枚 split-mode layer の cross-GPU 同期コストと解釈可能 (Vulkan 4 枚系の memory-bound 特性と整合、[pwr_sweep レポート](2026-06-26_081718_mi25_4card_load_vulkan_pwr_sweep.md) の観測値と一致)。pp_tps mean=586.08 t/s は 4 枚並列 prompt processing の期待値内
...
- llama.cpp: build-vulkan (RADV/master、mi25 skill 準拠のビルド)
...
### 1. 制御ホスト電源断中の mi25 llama-server 孤児化 — 5h 単独推論継続、GPU 側 fault 0

18:52:36 JST 直後の制御ホスト電源断で run_campaign プロセスが死んだ後、mi25 側の llama-server (PID 64542/64543) は **HTTP 接続切断を受けたが `--n-predict 32768` の最終 token まで推論を継続**、23:50 JST の Session2 起動前確認時点で `task 758927` が `n_decoded=921` まで進んで実行中だった。**5h の孤児推論中も dmesg amdgpu fault / GPU reset 0 件**、GPU[0]/GPU[1] 使用率 45-46% で正常な推論負荷。

**含意**:
- llama.cpp の `--parallel 1` + `--n-predict N` は HTTP client 切断を検知せず単独で `n_predict` 完了まで推論する挙動
- multi-GPU 4 枚同時負荷を 5h 継続しても fault は誘発されない = **D-2 R1 の fault=0 結論を強化する副次データ** (統計本体には集計しないが、safety margin としては +5h の 4 枚同時稼働で 0 fault)
- 運用示唆: campaign 中断時は llama-server も明示的に kill しないと VRAM を占有し続ける (今回は復旧時に `pkill` で対処済み)
...
- pp_tps mean = 586.08 t/s → 4 枚並列 prompt processing の期待範囲

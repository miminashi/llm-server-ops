...
mi25 サーバの GPU 4 枚のうち、過去のセッションで「fault が集中して発生する個体」として特定された 1 枚を、以前観測した位置 (旧 SLOT) から別の位置 (SLOT8) へ物理移動した状態で、4 枚を同時に稼働させる長時間負荷試験を実施した。目的は、この個体を含めた 4 枚 64GB 構成が本番運用に耐えるかを見極め、これまで安全策として 1 枚除外して運用してきた 3 枚 48GB 構成を卒業できるかを判断すること。

試験は 24 時間の連続負荷を目標に開始したが、開始から約 17 時間経過した時点で **制御ホスト側の建物でブレーカーが落ちて電源断** となり、途中で中断した。**mi25 側は別拠点にあり電源は継続していた** ため GPU の状態は保全され、5 時間後に制御ホストが復旧してから残りの試行を継続する第 2 セッションを実行、合わせて累計約 22 時間の実走で目標の試行数に到達した。

結果として、**期間全体を通して問題の個体を含む 4 枚全てで一度も fault が発生せず、GPU の物理層エラー、リセット、ハングもすべてゼロ**だった。統計的にも、過去に高頻度の fault を観測した比較対象 (旧 SLOT での 4 枚同時運用) に対し「同水準の発生率であればほぼ確実に 1 件以上観測されるはずの試行数」を蓄積した上でゼロだったため、当該比較対象の水準はほぼ棄却できる強い証拠が得られた。ただし過去に単独可視化構成 (問題の個体のみ稼働) で観測された低頻度 fault (~1.36%) の水準は、今回の試行数では統計的に棄却しきれないため、この水準の完全な否定は追加試験を要する。

これらの結果を受けて、**今後の常用構成としては 4 枚 64GB を採用してよい**と暫定判定した。従来の 3 枚 48GB 構成は「安全策の fallback」に位置付けを下げる。ただし低頻度水準の完全棄却は追加試験を要するため、余裕があれば追加走行 (R2) で確度を高める。

副次的に、以前のレポートで「c48c4 個体を SLOT8 に移した後、単独可視化構成では prompt 処理速度が半減する」という原因未解明の観察が記録されていたが、今回の 4 枚同時運用ではこの半減現象は再現せず、むしろ元の SA 期の速度を上回る回復が見られた。つまり **prompt 半減は単独可視化構成に固有の現象** であることが分かった。ただし単位時間あたりに完了できる推論リクエスト数 (実運用スループット) は SLOT8 SA 構成とほぼ同水準で、eval_tps の低下と pp_tps の上昇がバランスして全体では変わらない結果となった。したがって **4 枚並列運用の主なメリットは VRAM 容量拡大 (16GB → 64GB)** であり、スループット面ではほぼ中立。GPU の温度・電力・PCIe いずれの物理指標も、問題個体を含めた 4 枚とも他の健全個体と同水準で推移した。

運用面の教訓として、制御ホスト側でブレーカーが落ちるという想定外事象で campaign スクリプトが強制終了しても、mi25 側の推論プロセスは HTTP 切断を検知せず in-flight のリクエストを最後まで走らせて VRAM を占有し続けることが判明した。今後の resilience 設計では、制御ホストの復旧手順に「mi25 側の孤児プロセスを明示的に停止する」ステップを含める必要がある。

次セッションのタスクは、優先順に (1) 追加試験による低頻度水準の完全棄却 (任意)、(2) 前セッションから引き継いだ BMC 時刻同期の恒久設定、(3) Fable レビューの残タスクである fault シグネチャ台帳の再監査と 4 枚間の VBIOS/RAS カウンタ比較、(4) VBAT 監視の日次運用整備、の順で対応する。
...
- vs c48c4×SLOT6 4-card (3/88=3.41%): p = **0.1023** (5% にも 10% にも僅かに届かず、境界近傍。ただし検出力 96.78% で「真の rate が 3.41% なら 96.8% の確率で 1 件以上観測されるはず」= 実質棄却の強い証拠。R2 追試で N≥200 到達で 5% 有意到達可)
...
- **turns/trial = 4.97 (SLOT8 SA 5.0 とほぼ同水準)**: 実運用スループットは trials/h 4.44 vs SLOT8 SA 4.59 (-3.3%)、turns/h 22.29 vs 22.93 (-2.8%) で **実質同水準 (僅かに低下傾向)**。eval_tps -24% と pp_tps +15-166% がバランスして全体では変わらず → **4 枚並列運用の主メリットは VRAM 容量 (16GB → 64GB) のみ、スループット面ではほぼ中立**
...
**中断原因**: 制御ホスト側でブレーカー電源断が発生 (18:52:36 JST 直後)、run_campaign プロセス死亡。**mi25 側は完全健全** (別拠点で電源継続)、llama-server プロセスは孤児化して 5h 生存 (23:50 JST の Session2 起動前確認時点で task 758927 の `n_decoded=921` を llama_server.log 記録に確認、詳細な解釈は副次発見 1 参照)。
...
- 1h 毎 cron の集計 (trial 開始数 = Session1 の 75 + Session2 の 25 = 100) で「累計 N=100 到達 && fault=0」判定 → 完了通知、CronDelete、Phase 3 に進むことをユーザに提示 → ユーザ承認
- ※ Phase 3 の精査で `trial_done` event 単位の完了 trial 数 = **99** (Session1 で trial 75 は開始したが電源断で completion event 出ず、統計から除外) と確定。統計解析は N=99 で行う
...
**技術的解釈**:
- llama.cpp の `--parallel 1` は 1 slot を逐次処理、HTTP client 側の abort/切断を検知しない。in-flight request は effective cap (**load_driver.py が chat completions API に送る `max_tokens` = 2048 が先に効き、llama-server 側の `--n-predict 32768` は実質未到達**) まで走った後 idle 状態に戻る。従って「5h 単独推論継続」ではなく「in-flight task 完了 → 以降 idle で VRAM 占有」が正確な描像 (23:50 の n_decoded=921 の観察は task 完了直前か、その後 idle 期に llama_server.log から拾った過去ログを誤って現在時刻と解釈した可能性、精査未完)
...
### 4c. turns/trial は SLOT8 SA と同水準 (4.97 vs 5.0) — 実運用スループットは 4 枚並列でも変わらず

**当初「turns/trial +33%」と誤集計した後、精査で撤回**。正しい計算は以下の通り:

| 試験構成 | trial 数 | turn 総数 | turns/trial mean |
|---|---|---|---|
| Session1 (D-2 R1) | 75 (trial 1-75、うち 74 完了 + 75 部分) | 372 | 4.96 |
| Session2 (D-2 R1) | 25 (trial 1-25、全 25 完了) | 125 | 5.00 |
| **D-2 R1 合算** | **100 (unique trial 番号)** | **497** | **4.97** |
| SLOT8 SA (2026-07-04) | 221 | ~1105 (est.) | ~5.0 |
| a48e4 SLOT6 SA D-1 | 221 | ~1105 (est.) | ~5.0 |

**誤集計の原因**: Session1 と Session2 で trial 番号が独立にリセット (両方 1 から開始) するため、trial 番号だけで集計すると Session1 と Session2 の trial 1-25 が二重合算されて mean が押し上げられていた (誤: 497/75=6.63、正: 497/100=4.97)。

**実運用スループット (rate) 比較**:

| 指標 | D-2 R1 (99 trial / 22.3h) | SLOT8 SA (221 trial / 48.2h) | 差 |
|---|---|---|---|
| trials/h | 4.44 | 4.59 | **-3.3%** |
| turns/h | 22.29 | 22.93 | **-2.8%** |

**含意**: eval_tps -24% と pp_tps +15-166% が相殺し、**単位時間あたりのスループットは 4 枚並列と SLOT8 SA で実質同水準 (D-2 R1 が僅か -3% 低下傾向)**。したがって **4 枚 64GB 運用の主メリットは VRAM 容量拡大 (16GB → 64GB) のみ**、スループット面での積極的優位はなく、pp 重視ワークロード (RAG / 長 prompt) では pp_tps 上昇分だけ有利、eval 重視 (長文生成) では eval_tps 低下分だけ不利になる。ワークロード特性に応じた選択が可能。

**副次観測**: completion_tokens は mean 2038 / p50 2048 / max 2048 で **load_driver.py が chat completions API に送る `max_tokens=2048` が effective cap** (llama-server 側の `--n-predict 32768` は実質未到達)。副次発見 1 の「llama.cpp の孤児推論」も上限は 2048 で、多くの場合 2048 到達 → task 完了 → idle 状態に戻ってから VRAM 占有継続。

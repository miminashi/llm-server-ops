...
- **`head_count_kv` 修正の実機確認**: `fe3187de7` 以降なら `Shard_Rewrite/` の shard 1 が**未パッチのまま読めるはず**だが、コードを読んだだけで**実機未確認**。次回 GLM を触るときに確かめる。あわせて `UD-IQ4_XS-rewrite/` のパッチ済み shard 1 は**現 HEAD では有害**なので差し戻すこと

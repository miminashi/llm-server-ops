...
新しい本命は今回初めて実機に載せた。量子化モデルの配布元が用意した書き換え済みメタデータを差し替えれば読めるはずだったが、層ごとの設定値を並べた配列の長さが合わず失敗した。変換方式の変更に配布元のファイルが追随していないためで、配列の末尾を一要素落とす小さな修正で読み込め、モデル本体はそのまま流用できた。なおこの不具合は検証の翌日に PR 側で修正された。修正はこちらの回避策とは逆向きだったため、用意していた報告は投稿を見送り、回避策も現行版には使えないものとして記録している。
...
### 項目 1: #27773 の GGUF 互換性 — **`Shard_Rewrite` だけでは足りない**（`7de5a8e39` 時点）

> **追記（2026-09-02）**: 本項の不具合は `fe3187de7` で修正された。以下は `7de5a8e39` 時点の記録であり、**現 HEAD では 46 要素のまま読めるはず**（実機未確認）。詳細は「上流への動作報告」節。
>
> **追記（2026-09-03）**: **実機で確認した。** `2b533e0950` で未パッチの `Shard_Rewrite/` shard 1 は**そのまま読め**、逆に本項のパッチ済み shard 1 は `expected 46, got 45` で**落ちる**。予測は両方向とも裏づけられた。ただし**修正コミットの特定は不正確だった** — 変換側は PR 内 `9fe9fd71f8`「Repad n_head_kv」、**ローダ側は本家 master にマージされた #28173** であり、`fe3187de7` はその後のクリーンアップである。詳細は [#27773 の GGUF 受理条件が反転したことを実機で確認](./2026-09-03_035920_glm53flash_pr27773_head_recheck.md)。
...
### 結論: 報告は見送った。不具合は翌日に上流で修正された

**本節で扱う `head_count_kv` の不具合は、検証の翌日に PR 側で修正された。** 投稿用の文面を
用意していたが、**投稿は見送った**。

修正コミットは **`fe3187de7`「Review cleanup」（2026-09-02 15:53 +0200 = 22:53 JST）**。
検証に使った `7de5a8e39` との差分は 2 か所で、**46 を受け入れる側に倒している**。

変換スクリプト側 — 配列を `block_count`（46）まで埋めるようになった。
...
ローダ側 — 読み取る長さを `n_layer()`（45）から `n_layer_all`（46）へ変更。
...
パディングに使う値が `1` なのも、unsloth のファイルの 46 個目（`1` = NextN ブロックが DSA 層）と
一致する。**現 HEAD なら `Shard_Rewrite/` の shard 1 をそのまま読めるはず**である。

**重要: 本レポートのワークアラウンド（45 に切る）は現 HEAD では逆に有害になる。**
現 HEAD は 46 を期待するので、パッチ済み shard 1 を使うと今度は
`expected 46, got 45` で落ちるはずである。`UD-IQ4_XS-rewrite/` を再利用する場合は、
**未パッチの `Shard_Rewrite/` に戻すか、素の `UD-IQ4_XS/` を使うこと**。

**留保**: 上記はコードを読んだ結果であって**実機で確認していない**（両機は電源断済み）。
`fe3187de7` と投稿時刻の前後関係、および修正が投稿を受けたものかどうかは**判断材料がなく不明**。

**この経過自体が教訓である。** 活発な PR を検証する場合、**検証・レポート作成・投稿のあいだに
上流が動く**。投稿直前に対象コミットの差分を確認する手順が要る。今回は英訳の直前に
現 HEAD を確認したことで、既に直っている不具合の報告と、有害になったワークアラウンドの
公開を防げた。

### 経緯: 起草した英語の投稿が spam としてマークされた

本セッションで Claude が英語の投稿本文を起草し、ユーザが #27773 に投稿したところ
**spam としてマークされた**。マークしたのが自動フィルタなのかメンテナの手動操作なのかは
**判断材料がなく不明**である（外部リンクと `Authorization: Bearer` を含む長文が自動検出に
かかった可能性もある）。以下はその後にポリシー文書を読み直した結果である。

### llama.cpp の AI ポリシーは、どこに何が書いてあるか

**バグ報告への適用がいちばん明確なのは `.github/ISSUE_TEMPLATE/` である。** 6 つの
テンプレート全部（バグ報告 3 種を含む）に同じ一文が入っている。

> Please fill out this template **yourself**, copypasting language model outputs is
> [**strictly prohibited**](https://github.com/ggml-org/llama.cpp/blob/master/CONTRIBUTING.md#ai-usage-policy).

**ただしこれは issue フォームにしか表示されない。PR のコメント欄には出ない。**
今回投稿したのは PR のコメント欄なので、**目に入らない場所にしかルールが書かれていなかった**
ことになる。

`CONTRIBUTING.md` の AI Usage Policy 第 5 項にも記述がある。
...
**bug reports が明示されている**ので条文としては適用される。ただし読み方には注意が要る。
この 1〜5 のリストは

> If AI is used to generate any portion of **the code**, contributors must adhere to the following requirements:

という前置きの下にあり、1〜4 は「既存 PR を確認しろ」「PR 提出前にレビューしろ」
「提出したコードの全行を説明できるようにしろ」と**すべて PR 前提**である。5 番だけが
投稿文の話になる構造なので、**コードを一行も出していない人が「自分には関係ない」と読むのは
無理のない解釈**である。

**`AGENTS.md` はバグ報告を禁じていない。** 禁止リストは

> ### Prohibited AI Usage (results in immediate PR closure)
> - AI-written PR descriptions, commit messages, or reviewer responses

で **bug report は入っておらず**、制裁も `immediate PR closure` なので PR を出していない人には
効かない。同文書の "Communicate directly - verbose, AI-sounding responses will not be
well-received" は**書きぶりへの要求であって禁止条項ではない**。

**注意**: `CONTRIBUTING.md` には
`Undisclosed AI usage may result in your account being permanently banned` ともある。
**開示さえすれば AI 支援そのものは許される**（`AI-generated code is allowed`）。
禁じられているのは **モデルの出力をそのまま貼ること**である。

### 構造的な観察: 両文書は追試・バグ報告型の貢献者を想定していない

- 冒頭の貢献者 3 階層（Contributors / Collaborators / Maintainers）が**コード貢献の量で定義**されている
- `Before you start` 以下が丸ごと PR の話
- `Bug-fix PRs must include a reproducible issue` と**再現可能な issue を要求してはいる**のに、
  その issue を書く側への案内は「テンプレートを自分で埋めろ」以上に無い
- **「他人の PR を珍しいハードで試して結果を報告する」という貢献類型への言及がゼロ**

しかもこの類型は現に機能している。#27754 / #27752 のスレッドは feni6 の Metal での境界探索、
matteoscalabrini の 5x3090 での検証、Suaroman の vision 検証、tobieapb の MLX での対照実験と、
**まさに追試報告で回っている**。テンプレートの一文が狙っているのは中身のない LLM 生成 issue の
氾濫であって実測値を持ち込む人ではないと思われるが、条文は書き分けていない。

**したがって PR スレッドでの追試報告の作法は、文書ではなく上記の実例から学ぶほうが速い**
（1 投稿 1 主題、表は使うが前置きは短い、断定と推測を分ける、自分の環境の限界を明記する）。

### 運用方針

**Claude は投稿本文を書かない。素材出しに留める。** エラー文字列・原因の切り分け・確認済みの
数値までを出し、**投稿する文章は人間が自分の言葉で書く**。英語が書けない場合も、完成文ではなく
**平易な文型と言い換え候補**を出し、書き換えを前提にする。

**説明できないことは書かない。** 今回ユーザはローダのコードを理解していないので、
**仕組みの説明は投稿に含めない**。エラーメッセージ自体が `expected 45, got 46` と言っており、
受け取るのは変換スクリプトを書いた本人なので、**なぜ 45 なのかを報告者が説明する必要はない**。

### 投稿の骨組み（合意した 6 項目）

| # | 内容 |
|---|---|
| 1 | #27773 を 13x Tesla P100 の RPC 分散で試した。モデルは unsloth の `UD-IQ4_XS`、shard 1 は danielhanchen が用意した `Shard_Rewrite/` のもの。コミットは投稿時点で実際に試したもの |
| 2 | そのままでは読み込みに失敗した（`key glm5-next.attention.head_count_kv has wrong array length; expected 45, got 46` を 1 行だけ引用） |
| 3 | Claude に調べてもらったところ、GGUF 側の配列が 46 要素、この PR の `conversion/glm.py` は 45 要素という食い違いだった |
| 4 | Claude が shard 1 のメタデータのその 1 か所だけ 45 に揃えた。**llama.cpp のコードは無変更、重みも無変更** |
| 5 | それで読み込めて、そのあとは普通に動いた |
| 6 | 開示: 調査も修正も Claude によるもので、自分はローダのコードを理解していない。診断は結論ではなくデータ点として扱ってほしい |

**投稿しないもの**と、その理由。

| 落とすもの | 理由 |
|---|---|
| KDA / DSA、NextN ブロックの解説 | 説明を求められて答えられない。著者には不要 |
| `swiglu_clamp_*` は 46 で一致している話 | 同上。著者は自分のコードなので言われなくても分かる |
| バイト並び、パッチスクリプト | 求められてから出す |
| 「ローダが直すべきか配布側が直すべきか」 | 意見を持てない問いは聞かない |
| `-ub` や 108k の数値（動作報告） | spam マークの直後に別投稿を重ねない。求められてから |

**書き方**: 見出しを使わない、表を使わない、太字リード箇条書きにしない、em dash を多用しない、
`Happy to ...` のような定型の申し出を付けない。**短くて素っ気ないほうが信用される。**

### 聞き返されたときの答え

- 仕組みは分からない。Claude の分析なので確認して答える
- パッチは出せる（4 バイトの編集）
- 必要なら `Shard_Rewrite/` のファイルで再現し直して確認する

**「分からない」と言える形にしておくのが、この投稿でいちばん重要な設計である。**

### 投稿前に確認すること（今回は投稿に至らなかったが、次回のために）

- **投稿直前に対象 PR の HEAD の差分を必ず確認する。** 今回はこれで「既に修正済み」を掴んだ。
  検証コミットと投稿時点の HEAD がずれている前提で動くこと
- **本文は人間が書く。** Claude は技術的事実の照合（コミットハッシュ、エラー文字列、45 と 46 の
  向き）だけを行い、英語の推敲はしない。**日本語で書いてから訳すと書きやすい**（ユーザの気づき）
- **コミットハッシュを必ず入れる。** どの時点の話かが決定的に効く（今回がまさにその例）
- **「コードは変えていない」と書く。** 「他のコードはいじっていない」だと何かコードを触ったように
  読まれる。実際に変えたのは GGUF のメタデータであってコードではない
- spam マークされた既存コメントについては、**「前の投稿は AI が書いたもので、ポリシーを理解して
  いなかった。これは自分で書いた」という一行は入れてよい**。反論ではなく、なぜこの投稿が違うのかの
  説明になるため（当初「既存コメントには触れない」と助言したが、これは訂正する）
...
- **活発な PR を検証すると、検証・レポート作成・投稿のあいだに上流が動く**。本レポートの主題だった `head_count_kv` の不具合は、**検証の翌日 `fe3187de7`（2026-09-02 22:53 JST）で修正された**。投稿用の英訳をする直前に HEAD の差分を確認したことで、既に直っている不具合の報告を防げた。さらに**本レポートのワークアラウンド（45 に切る）は現 HEAD では逆に落ちる**はずなので、公開していたら追随した人を壊していた。**投稿直前に対象コミットの差分を見る手順を必ず挟むこと**
- **投稿前に対象リポジトリの AI ポリシーを読むこと**。本セッションで起草した英語の投稿本文は #27773 で **spam としてマークされた**。llama.cpp では**バグ報告への適用がいちばん明確なのは `.github/ISSUE_TEMPLATE/` の「テンプレートは自分で埋めろ、モデル出力の貼り付けは禁止」の一文**だが、**これは issue フォームにしか表示されず PR のコメント欄には出ない**。`CONTRIBUTING.md` 第 5 項にも bug reports の明記はあるが「コードを AI に生成させた場合」という前置きの下にあり、`AGENTS.md` の 禁止リストに bug report は入っていない。**禁じられているのはモデル出力をそのまま貼ることで、開示すれば AI 支援自体は許される**。詳細は「上流への動作報告」節
...
- ~~**`head_count_kv` 修正の実機確認**~~ → **2026-09-03 に決着**。`2b533e0950` で未パッチが読め、パッチ済みは `expected 46, got 45` で落ちることを実測。`UD-IQ4_XS-rewrite/` の shard 1 も未パッチ版に差し戻し済み。[続編レポート](./2026-09-03_035920_glm53flash_pr27773_head_recheck.md)

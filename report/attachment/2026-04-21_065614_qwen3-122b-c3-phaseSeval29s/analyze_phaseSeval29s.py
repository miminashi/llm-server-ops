#!/usr/bin/env python3
"""analyze_phaseSeval29s.py - Phase S-eval-29session 集計スクリプト

29 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval28s + 本 Phase phaseSeval29s) を合算し、
ub 別 n=29 の session 間 σ, 29-session verdict, ピーク順序安定性、pooled 145-run 統計、
...
CUR_SESSION_LABEL = "S29_phaseSeval29s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval29s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval29s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval29s_verdict.txt"
    with verdict_path.open("w") as f:
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S29) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 29 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 28-session pool vs cur S29)
        f.write("\n## 5. Prior 28-session pool (S1..S28) vs 本 Phase (S29) Welch t\n")
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (29 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (29 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (29 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (29 session)\n")

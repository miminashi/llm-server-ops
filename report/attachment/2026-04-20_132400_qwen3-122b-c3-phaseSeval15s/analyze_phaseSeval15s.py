#!/usr/bin/env python3
"""analyze_phaseSeval15s.py - Phase S-eval-15session 集計スクリプト

15 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval13s + phaseSeval14s + 本 Phase phaseSeval15s) を合算し、
ub 別 n=15 の session 間 σ, 15-session verdict, ピーク順序安定性、pooled 75-run 統計、
ub=1584/1586/1664 崩壊頻度 (Wilson 95% CI)、ub 別時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 / S8 / S9 / S10 / S11 / S12 / S13 / S14 / S15 のモード分類比較を出力する。
"""
...
CUR_SESSION_LABEL = "S15_phaseSeval15s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval15s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval15s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval15s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-15session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S15) 5-run 統計（eval フェーズ）\n")
        ...
        # 5. Welch t (prior 14-session pool vs cur S15)
        ...
        f.write("\n## 9. 15-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (15 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (15 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (15 session)\n")

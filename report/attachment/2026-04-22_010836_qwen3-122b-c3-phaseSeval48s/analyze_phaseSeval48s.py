#!/usr/bin/env python3
"""analyze_phaseSeval48s.py - Phase S-eval-48session 集計スクリプト

48 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval47s + 本 Phase phaseSeval48s) を合算し、
ub 別 n=48 の session 間 σ, 48-session verdict, ピーク順序安定性、pooled 240-run 統計、
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval48s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval48s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval48s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-48session verdict\n")
        ...
        # 1. 本 Phase (S48) 5-run 統計
        f.write("## 1. 本 Phase (S48) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 48 session mean 時系列
        f.write("\n## 2. 48 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 47-session pool vs cur S48)
        f.write("\n## 5. Prior 47-session pool (S1..S47) vs 本 Phase (S48) Welch t\n")
        ...
        # 9. 48-session verdict summary
        f.write("\n## 9. 48-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (48 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (48 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (48 session)\n")
        ...
        # 12. ピーク順序 48-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (48 session)\n")

#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 59 session + Sbfine 3 reference points の時系列プロット
...
- S1..S59 (5-run mean) を x=1..59 に折れ線 + 丸 marker でプロット
- 崩壊閾値 eval_mean=15.0 を水平線で表示
- S1..S59 に対する線形回帰直線（trend line）を各 ub ごとに dashed で重ねる
"""
...
# S1..S59 summary TSV path (phase=eval のみ抽出) + S59 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S52", "2026-04-22_044633_qwen3-122b-c3-phaseSeval52s", "summary_phaseSeval52s.tsv"),
    ("S53", "2026-04-22_054754_qwen3-122b-c3-phaseSeval53s", "summary_phaseSeval53s.tsv"),
    ("S54", "2026-04-22_072412_qwen3-122b-c3-phaseSeval54s", "summary_phaseSeval54s.tsv"),
    ("S55", "2026-04-22_081858_qwen3-122b-c3-phaseSeval55s", "summary_phaseSeval55s.tsv"),
    ("S56", "2026-04-22_091115_qwen3-122b-c3-phaseSeval56s", "summary_phaseSeval56s.tsv"),
    ("S57", "2026-04-22_100502_qwen3-122b-c3-phaseSeval57s", "summary_phaseSeval57s.tsv"),
    ("S58", "2026-04-22_110239_qwen3-122b-c3-phaseSeval58s", "summary_phaseSeval58s.tsv"),
    ("S59", None, "summary_phaseSeval59s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval59s.tsv
]
...
def main() -> int:
    ...
    for ub in UBS:
        ...
        # 回帰直線 (S1..S59 only、Sbfine ref は除外)
    ...
    ax.set_title(
        "Phase S-eval 58-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

#!/usr/bin/env python3
"""analyze_phaseSeval59s.py - Phase S-eval-59session 集計スクリプト

58 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval57s + 本 Phase phaseSeval59s) を合算し、
ub 別 n=59 の session 間 σ, 59-session verdict, ピーク順序安定性、pooled 295-run 統計、
...
PRIOR_TSVS = [
    ...
    ("S53_phaseSeval53s",
     SCRIPT_DIR.parent / "2026-04-22_054754_qwen3-122b-c3-phaseSeval53s" / "summary_phaseSeval53s.tsv"),
    ("S54_phaseSeval54s",
     SCRIPT_DIR.parent / "2026-04-22_072412_qwen3-122b-c3-phaseSeval54s" / "summary_phaseSeval54s.tsv"),
    ("S55_phaseSeval55s",
     SCRIPT_DIR.parent / "2026-04-22_081858_qwen3-122b-c3-phaseSeval55s" / "summary_phaseSeval55s.tsv"),
    ("S56_phaseSeval56s",
     SCRIPT_DIR.parent / "2026-04-22_091115_qwen3-122b-c3-phaseSeval56s" / "summary_phaseSeval56s.tsv"),
    ("S57_phaseSeval57s",
     SCRIPT_DIR.parent / "2026-04-22_100502_qwen3-122b-c3-phaseSeval57s" / "summary_phaseSeval57s.tsv"),
    ("S58_phaseSeval58s",
     SCRIPT_DIR.parent / "2026-04-22_110239_qwen3-122b-c3-phaseSeval58s" / "summary_phaseSeval58s.tsv"),
]
CUR_SESSION_LABEL = "S59_phaseSeval59s"
...
MODE_GROUPS = {
    ...
    "prev_S53": ["S53_phaseSeval53s"],
    "prev_S54": ["S54_phaseSeval54s"],
    "prev_S55": ["S55_phaseSeval55s"],
    "prev_S56": ["S56_phaseSeval56s"],
    "prev_S57": ["S57_phaseSeval57s"],
    "prev_S58": ["S58_phaseSeval58s"],
    "cur_S59": ["S59_phaseSeval59s"],
}
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval59s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval59s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval59s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-59session verdict\n")
        ...
        # 1. 本 Phase (S55) 5-run 統計
        f.write("## 1. 本 Phase (S55) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 55 session mean 時系列
        f.write("\n## 2. 55 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 55-session pool vs cur S56)
        f.write("\n## 5. Prior 55-session pool (S1..S55) vs 本 Phase (S56) Welch t\n")
        ...
        # 9. 55-session verdict summary
        f.write("\n## 9. 55-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (55 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (55 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (55 session)\n")
        ...
        # 12. ピーク順序 55-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (55 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S55）\n")
        w1_1584 = None

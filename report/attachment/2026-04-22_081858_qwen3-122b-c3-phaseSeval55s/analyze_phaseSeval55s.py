#!/usr/bin/env python3
"""analyze_phaseSeval55s.py - Phase S-eval-55session 集計スクリプト

54 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval54s + 本 Phase phaseSeval55s) を合算し、
ub 別 n=55 の session 間 σ, 55-session verdict, ピーク順序安定性、pooled 275-run 統計、
...
CUR_SESSION_LABEL = "S55_phaseSeval55s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval55s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval55s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval55s_verdict.txt"
    with verdict_path.open("w") as f:
        ...
        # 5. Welch t (prior 54-session pool vs cur S55)
        f.write("\n## 5. Prior 54-session pool (S1..S54) vs 本 Phase (S55) Welch t\n")

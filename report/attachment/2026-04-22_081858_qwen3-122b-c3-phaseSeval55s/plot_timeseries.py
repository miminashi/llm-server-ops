#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 55 session + Sbfine 3 reference points の時系列プロット
...
- S1..S55 (5-run mean) を x=1..55 に折れ線 + 丸 marker でプロット
- 崩壊閾値 eval_mean=15.0 を水平線で表示
- S1..S55 に対する線形回帰直線（trend line）を各 ub ごとに dashed で重ねる
"""
...
# S1..S54 summary TSV path (phase=eval のみ抽出) + S55 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S55", None, "summary_phaseSeval55s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval55s.tsv
]
...
def main() -> int:
    ...
    for ub in UBS:
        ...
        # 回帰直線 (S1..S55 only、Sbfine ref は除外)

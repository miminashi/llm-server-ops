#!/usr/bin/env python3
"""analyze_phaseSeval13s.py - Phase S-eval-13session 集計スクリプト

13 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval11s + phaseSeval12s + 本 Phase phaseSeval13s) を合算し、
ub 別 n=13 の session 間 σ, 13-session verdict, ピーク順序安定性、pooled 65-run 統計、
ub=1584/1586/1664 崩壊頻度 (Wilson 95% CI)、ub 別時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 / S8 / S9 / S10 / S11 / S12 / S13 のモード分類比較を出力する。
"""
...
CUR_SESSION_LABEL = "S13_phaseSeval13s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval13s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval13s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval13s_verdict.txt"
    with verdict_path.open("w") as f:
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S13) 5-run 統計（eval フェーズ）\n")
        ...
        # 5. Welch t (prior 12-session pool vs cur S13)
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (13 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (13 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (13 session)\n")

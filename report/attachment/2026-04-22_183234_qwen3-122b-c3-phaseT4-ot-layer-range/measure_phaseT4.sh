#!/usr/bin/env bash
# measure_phaseT4.sh - Phase T-4 計測（OT pattern 層範囲スイープ）
# usage:
#   measure_phaseT4.sh <pid> <tag> <prompt_spec>
#     prompt_spec:
#       "文字列" → そのままプロンプト
...
log "==== measure_phaseT4 start tag=${TAG} pid=${PID} ===="
log "wait /health"
...
log "==== measure_phaseT4 end tag=${TAG} ===="

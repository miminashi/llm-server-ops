#!/usr/bin/env python3
"""analyze_phaseSeval49s.py - Phase S-eval-48session 集計スクリプト

49 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval48s + 本 Phase phaseSeval49s) を合算し、
ub 別 n=49 の session 間 σ, 49-session verdict, ピーク順序安定性、pooled 245-run 統計、
...
CUR_SESSION_LABEL = "S49_phaseSeval49s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval49s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval49s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval49s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-49session verdict\n")
        ...
        # 1. 本 Phase (S49) 5-run 統計
        f.write("## 1. 本 Phase (S49) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 49 session mean 時系列
        f.write("\n## 2. 49 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 48-session pool vs cur S49)
        f.write("\n## 5. Prior 48-session pool (S1..S48) vs 本 Phase (S49) Welch t\n")
        ...
        # 9. 49-session verdict summary
        f.write("\n## 9. 49-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (49 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (49 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (49 session)\n")
        ...
        # 12. ピーク順序 49-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (49 session)\n")

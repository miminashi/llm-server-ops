#!/usr/bin/env python3
"""analyze_phaseSeval35s.py - Phase S-eval-35session 集計スクリプト

35 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval34s + 本 Phase phaseSeval35s) を合算し、
ub 別 n=35 の session 間 σ, 35-session verdict, ピーク順序安定性、pooled 175-run 統計、
...
CUR_SESSION_LABEL = "S35_phaseSeval35s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval35s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval35s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval35s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-35session verdict\n")
        ...
        f.write("\n## 2. 35 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 34-session pool vs cur S35)
        f.write("\n## 5. Prior 34-session pool (S1..S34) vs 本 Phase (S35) Welch t\n")
        ...
        # 9. 35-session verdict summary
        f.write("\n## 9. 35-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (35 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (35 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (35 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (35 session)\n")

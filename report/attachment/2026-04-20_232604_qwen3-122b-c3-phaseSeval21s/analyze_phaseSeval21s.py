#!/usr/bin/env python3
"""analyze_phaseSeval21s.py - Phase S-eval-21session 集計スクリプト

21 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval20s + 本 Phase phaseSeval21s) を合算し、
ub 別 n=21 の session 間 σ, 21-session verdict, ピーク順序安定性、pooled 105-run 統計、
...
CUR_SESSION_LABEL = "S21_phaseSeval21s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval21s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval21s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval21s_verdict.txt"

#!/usr/bin/env python3
"""analyze_phaseSeval43s.py - Phase S-eval-43session 集計スクリプト

43 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval42s + 本 Phase phaseSeval43s) を合算し、
ub 別 n=43 の session 間 σ, 43-session verdict, ピーク順序安定性、pooled 215-run 統計、
...
CUR_SESSION_LABEL = "S43_phaseSeval43s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval43s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval43s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval43s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-43session verdict\n")
        ...
        # 1. 本 Phase (S43) 5-run 統計
        f.write("## 1. 本 Phase (S43) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 43 session mean 時系列
        f.write("\n## 2. 43 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 42-session pool vs cur S43)
        f.write("\n## 5. Prior 42-session pool (S1..S42) vs 本 Phase (S43) Welch t\n")
        ...
        # 9. 43-session verdict summary
        f.write("\n## 9. 43-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (43 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (43 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (43 session)\n")
        ...
        # 12. ピーク順序 43-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (43 session)\n")

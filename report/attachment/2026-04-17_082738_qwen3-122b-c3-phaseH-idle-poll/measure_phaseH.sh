#!/usr/bin/env bash
# measure_phaseH.sh - Phase H 計測（Phase F + 各 Run status + 追加 snapshots）
# usage: measure_phaseH.sh <pid> <tag>
...
log "==== measure_phaseH start tag=${TAG} pid=${PID} ===="
log "wait /health"
...
log "==== measure_phaseH end tag=${TAG} ===="

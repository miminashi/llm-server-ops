#!/usr/bin/env python3
"""analyze_phaseSeval45s.py - Phase S-eval-45session 集計スクリプト

45 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval44s + 本 Phase phaseSeval45s) を合算し、
ub 別 n=45 の session 間 σ, 45-session verdict, ピーク順序安定性、pooled 225-run 統計、
...
CUR_SESSION_LABEL = "S45_phaseSeval45s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval45s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval45s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval45s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-45session verdict\n")
        ...
        # 1. 本 Phase (S45) 5-run 統計
        f.write("## 1. 本 Phase (S45) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 45 session mean 時系列
        f.write("\n## 2. 45 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 44-session pool vs cur S45)
        f.write("\n## 5. Prior 44-session pool (S1..S44) vs 本 Phase (S45) Welch t\n")
        ...
        # 9. 45-session verdict summary
        f.write("\n## 9. 45-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (45 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (45 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (45 session)\n")
        ...
        # 12. ピーク順序 45-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (45 session)\n")

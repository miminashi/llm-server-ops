...
SCRATCH=/home/ubuntu/projects/llm-server-ops/report/attachment/2026-07-07_075528_mi25_a48e4_slot6_24h_round2
SERVER=mi25
...
recover_from_hang(){ # $1=hang index。成功:0 / 失敗:9
  ...
  ( cd "$PROJ" && .claude/skills/gpu-server/scripts/unlock.sh "$SERVER"; .claude/skills/gpu-server/scripts/lock.sh "$SERVER" "a48e4-slot6-24h-r2-2026-07-07_075528" ) >> "$CAMPLOG" 2>&1
  # 5) llama 再起動
  restart_llama
  # 6) /health 確認
...
record_boot_state "phase-start-${BACKEND}-a48e4-slot6-r2"
start_telemetry

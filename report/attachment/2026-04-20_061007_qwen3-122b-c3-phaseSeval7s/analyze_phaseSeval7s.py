#!/usr/bin/env python3
"""analyze_phaseSeval7s.py - Phase S-eval-7session 集計スクリプト

7 session (phaseSeval + phaseSevalcross + phaseSeval3s + phaseSeval4s + phaseSeval5s + phaseSeval6s + 本 Phase phaseSeval7s) を合算し、
ub 別 n=7 の session 間 σ, 7-session verdict, ピーク順序安定性、pooled 35-run 統計、
ub=1584/1586 崩壊頻度 (Wilson 95% CI)、ub=1664 時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 のモード分類比較を出力する。
"""
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval7s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval7s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval7s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-7session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S7) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 7 session mean 時系列
        f.write("\n## 2. 7 session mean 時系列 (eval 5-run mean, t/s)\n")
        f.write("ub    | S1_Seval | S2_Scross | S3_3s   | S4_4s   | S5_5s   | S6_6s   | S7_7s   | range | mean_of_7 | σ_session\n")
        f.write("------|----------|-----------|---------|---------|---------|---------|---------|-------|-----------|----------\n")
        ...
        for ub in UBS:
            ...
            if len(finite) >= 2:
                ...
                f.write(
                    f"{ub}  | {cells[0]} | {cells[1]} | {cells[2]} | {cells[3]} | {cells[4]} | {cells[5]} | {cells[6]} | "
        ...
        # 5. Welch t (prior 6-session pool vs cur S7)
        f.write("\n## 5. Prior 6-session pool (S1+S2+S3+S4+S5+S6) vs 本 Phase (S7) Welch t\n")
        ...
        # 6. pooled 35-run
        f.write("\n## 6. Pooled 35-run (S1+S2+S3+S4+S5+S6+S7) 統計\n")
        ...
        # 9. 7-session verdict summary
        f.write("\n## 9. 7-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (7 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (7 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (7 session)\n")
        ...
        # 12. ピーク順序 7-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (7 session)\n")
        ...
        # 13. モード分類 (S1-S3 / S4-S5 / S6 / S7) 比較
        f.write("\n## 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S7) の eval_mean 比較\n")
        f.write("ub    | modeA_mean | modeB_mean | modeC_mean | S7_mean | closer_to | dA     | dB     | dC\n")
        f.write("------|------------|------------|------------|---------|-----------|--------|--------|--------\n")
        for ub in UBS:
            mode_A = []
            mode_B = []
            ...
            sb = stats(mode_B)
            sc = stats(mode_C)
            s7 = stats(cur_eval[ub])
            if sa is None or sb is None or sc is None or s7 is None:
                f.write(f"{ub}  | insufficient data\n")
                continue
            dA = s7["mean"] - sa["mean"]
            dB = s7["mean"] - sb["mean"]
            dC = s7["mean"] - sc["mean"]
            d_abs = {"mode_A": abs(dA), "mode_B": abs(dB), "mode_C": abs(dC)}
            closer = min(d_abs, key=d_abs.get)
            f.write(f"{ub}  | {sa['mean']:10.3f} | {sb['mean']:10.3f} | {sc['mean']:10.3f} | {s7['mean']:7.3f} | "
                    f"{closer:9s} | {dA:+.3f} | {dB:+.3f} | {dC:+.3f}\n")

        # 14. warmup1 ub=1584 absolute 値のモード判定
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S7）\n")
        w1_1584 = None
        ...
        if w1_1584 is not None and eval_1584 is not None:
            ...
            f.write(f"  判定: {band}\n")
            # Δ 帯判定 (A +0.30〜0.31 / B +0.15〜0.16 / C +0.017)
            if abs(delta - 0.017) < 0.05:
                ...
                dband = "mode_A_delta (S1-S3: +0.30〜+0.31)"
            else:

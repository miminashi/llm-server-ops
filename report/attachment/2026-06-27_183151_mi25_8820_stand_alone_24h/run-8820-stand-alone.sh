#!/bin/bash
# 8820 単独可視化 + Vulkan + Qwen3-8B Q6_K で llama-server を起動するスタンドアロン起動コマンド。
...
# 8820 = Vulkan idx 3 のみ可視化 (multi-GPU 経路を排除)
GGML_VK_IDX="3"
...
echo "==> llama-server 起動 (8820 stand-alone Vulkan ctx=$CTX_SIZE)"

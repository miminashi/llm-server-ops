#!/usr/bin/env bash
# measure_phaseD.sh - Phase D の軽量計測（dmon + curl eval + status + numastat snap）
# usage: measure_phaseD.sh <pid> <tag>
...
log "==== measure_phaseD start tag=${TAG} pid=${PID} ===="
log "wait /health"
...
log "==== measure_phaseD end tag=${TAG} ===="

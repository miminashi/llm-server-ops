#!/usr/bin/env python3
"""analyze_phaseSeval23s.py - Phase S-eval-23session 集計スクリプト

23 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval22s + 本 Phase phaseSeval23s) を合算し、
ub 別 n=23 の session 間 σ, 23-session verdict, ピーク順序安定性、pooled 115-run 統計、
...
CUR_SESSION_LABEL = "S23_phaseSeval23s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval23s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval23s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval23s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-23session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S23) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 23 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        f.write("\n## 5. Prior 22-session pool (S1..S22) vs 本 Phase (S23) Welch t\n")
        ...
        f.write("\n## 9. 23-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (23 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (23 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (23 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (23 session)\n")

#!/usr/bin/env bash
# start_phaseJ.sh - Phase J flash-attn A/B 比較用 (C-D3 採用構成をベース)
# FLASH_ATTN 環境変数で 0/1 を切り替え可能にした以外は start_phaseI.sh と同一
...
echo "[start_phaseJ] FLASH_ATTN=${FLASH_ATTN} (C-D3 base, poll=0, ctx=131072)"
echo "[start_phaseJ] launch cmd:"
echo "$LAUNCH_CMD"

ssh -f "$HOST" "cd ~/llama.cpp && nohup bash -c \"${LAUNCH_CMD}\" > /tmp/llama-server.log 2>&1 < /dev/null &"

echo "[start_phaseJ] waiting for /health..."
for i in $(seq 1 60); do
  if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    echo "[start_phaseJ] /health OK after ${i}*5s"
    sleep 5
    PID=$(ssh "$HOST" "ps -eo pid,comm,args | awk '\$2==\"llama-server\" {print \$1; exit}'")
    echo "[start_phaseJ] PID=${PID}"
    echo "$PID"
    exit 0
  fi
  sleep 5
done
echo "[start_phaseJ] FAILED to become healthy in 300s" >&2
exit 1

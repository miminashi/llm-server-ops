#!/bin/bash
# mi25 c48c4 (SLOT8 = BDF 84:00.0 = GPU[2]) 含む 4 枚同時装着 24h 負荷キャンペーン オーケストレータ。
# Fable レビュー D-2 決定実験: c48c4 SLOT8 SA (0/221) が multi-GPU 環境で維持されるかを検証。
# run_campaign_a48e4.sh (D-1) から派生:
#   - Vulkan idx 3 → "0,1,2,3" (4 枚可視化) → run-c48c4-slot8-4card.sh 経由
#   - ROCM_DEVICE_IDX_LIST=(0 1 2 3) で power cap 監視を 4 枚全てに拡張
#   - MAX_TRIALS=120 / MIN_TRIALS=100 / HANG_SAFETY=5 (ユーザ選択の N≥100 に合わせて縮小)
#
# 試行ループを回し、ハング時は KVMスクショ→BMCリセット→復帰→ブート状態記録→ロック再取得
# →llama再起動→テレメトリ再起動 を自動化して継続する。
#
# Usage:
#   run_campaign_c48c4_4card.sh
#   主要パラメータは環境変数で上書き可:
#   MAX_TRIALS(120) MIN_TRIALS(100) HANG_SAFETY(5) PHASE_CAP_SEC(86400=24h) TRIAL_SEC(720)
#   CTX_SIZE(131072 = run-c48c4-slot8-4card.sh 経由でラッパに渡す)
#
# 終了コード: 0=正常完了 / 7=ハング安全境界に到達(ユーザ確認要) / 9=復旧失敗(要介入)

set -u
PROJ=/home/ubuntu/projects/llm-server-ops
SCRATCH=/home/ubuntu/projects/llm-server-ops/report/attachment/2026-07-18_020309_mi25_c48c4_slot8_4card_24h_r1
SERVER=mi25
ENDPOINT=http://10.1.4.13:8000
MODEL="unsloth/Qwen3-8B-GGUF:Q6_K"
BACKEND="vulkan"
# 4 枚 (0=c3164 / 1=448c4 / 2=c48c4 / 3=a48e4) 全ての power cap を監視・再設定対象に
ROCM_DEVICE_IDX_LIST=(0 1 2 3)

MAX_TRIALS="${MAX_TRIALS:-120}"
MIN_TRIALS="${MIN_TRIALS:-100}"
HANG_SAFETY="${HANG_SAFETY:-5}"
...
record_boot_state(){ # $1=reset_type
    ...
    "rocm-smi --showmaxpower 2>/dev/null | grep -oE 'GPU\[[0-9]+\]' | sort -u | wc -l" 2>/dev/null)
  {
    ...
    echo "--- rocm-smi --showuniqueid --showbus (unique ID <-> BDF) ---"
    ssh -o ConnectTimeout=8 -o BatchMode=yes "$SERVER" "rocm-smi --showuniqueid --showbus 2>/dev/null" 2>&1
    echo "--- root fs ---"
...
restart_llama(){
  log "llama 再起動 (c48c4 SLOT8 4-card Vulkan ctx=$CTX_SIZE)"
  # 4 枚可視化ラッパを呼ぶ (start.sh の detect_radv_vk_indices を回避)
  CTX_SIZE="$CTX_SIZE" bash "$SCRATCH/run-c48c4-slot8-4card.sh" >> "$CAMPLOG" 2>&1
  local rc=$?
  # BACO 後の power cap 160W 強制再設定 (stand_alone_24h 副次発見: BACO で 220W に戻る)
  # 4 枚全てをチェックして必要な枚だけ再設定
  if [ "$rc" -eq 0 ]; then
    local idx pc
    for idx in "${ROCM_DEVICE_IDX_LIST[@]}"; do
      pc=$(ssh -o ConnectTimeout=8 -o BatchMode=yes "$SERVER" \
        "rocm-smi --showmaxpower 2>/dev/null | grep -E \"GPU\\[${idx}\\].*Power\" | grep -oE '[0-9]+\\.[0-9]+' | head -1" 2>/dev/null)
      if [ -n "$pc" ] && awk -v p="$pc" 'BEGIN{exit !(p>165)}'; then
        log "BACO power cap reset detected (GPU[$idx] cap=${pc}W) → 160W に再設定"
        ssh -o ConnectTimeout=8 "$SERVER" \
          "sudo rocm-smi --setpoweroverdrive 160 -d $idx" >> "$CAMPLOG" 2>&1
      fi
    done
  fi
  return $rc
...
recover_from_hang(){ # $1=hang index。成功:0 / 失敗:9
  ...
  ssh -o ConnectTimeout=8 "$SERVER" "sudo journalctl -b -1 --no-pager 2>/dev/null | grep -iE 'amdgpu|gpu reset|ring|pcie|aer|soft lockup|nmi|panic|call trace' | tail -100" \
    >> "$SCRATCH/prevboot_${BACKEND}_${hn}.log" 2>&1
  ( cd "$PROJ" && .claude/skills/gpu-server/scripts/unlock.sh "$SERVER"; .claude/skills/gpu-server/scripts/lock.sh "$SERVER" "c48c4-slot8-4card-24h-r1-2026-07-18_020309" ) >> "$CAMPLOG" 2>&1
  restart_llama
...
log "対象: c48c4 (SLOT8 BDF 84:00.0 GPU[2]) を含む 4 枚同時 (GGML_VK_VISIBLE_DEVICES=0,1,2,3)"
record_boot_state "phase-start-${BACKEND}-c48c4-slot8-4card"
start_telemetry

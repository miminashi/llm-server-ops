#!/usr/bin/env python3
"""analyze_phaseSeval20s.py - Phase S-eval-20session 集計スクリプト

20 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval19s + 本 Phase phaseSeval20s) を合算し、
ub 別 n=16 の session 間 σ, 19-session verdict, ピーク順序安定性、pooled 100-run 統計、
...
CUR_SESSION_LABEL = "S20_phaseSeval20s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval20s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval20s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval20s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-20session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S20) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 5. Prior 19-session pool (S1..S19) vs 本 Phase (S20) Welch t\n")
        ...
        # 9. 19-session verdict summary
        f.write("\n## 9. 19-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (20 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (20 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (20 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (20 session)\n")

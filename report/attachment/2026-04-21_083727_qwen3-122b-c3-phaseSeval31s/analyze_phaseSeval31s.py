#!/usr/bin/env python3
"""analyze_phaseSeval31s.py - Phase S-eval-31session 集計スクリプト

31 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval29s + 本 Phase phaseSeval31s) を合算し、
ub 別 n=29 の session 間 σ, 29-session verdict, ピーク順序安定性、pooled 155-run 統計、
...
CUR_SESSION_LABEL = "S31_phaseSeval31s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval31s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval31s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval31s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-31session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S31) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 31 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        f.write("\n## 5. Prior 29-session pool (S1..S29) vs 本 Phase (S31) Welch t\n")
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (31 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (31 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (31 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (31 session)\n")

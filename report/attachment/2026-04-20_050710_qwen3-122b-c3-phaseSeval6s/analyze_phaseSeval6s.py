#!/usr/bin/env python3
"""analyze_phaseSeval6s.py - Phase S-eval-6session 集計スクリプト

6 session (phaseSeval + phaseSevalcross + phaseSeval3s + phaseSeval4s + phaseSeval5s + 本 Phase phaseSeval6s) を合算し、
ub 別 n=6 の session 間 σ, 6-session verdict, ピーク順序安定性、pooled 30-run 統計、
ub=1584 崩壊頻度 (Wilson 95% CI)、ub=1664 時系列パターン分析、
S1-S3 vs S4-S5 vs S6 モード分類比較を出力する。
"""
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval6s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval6s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval6s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-6session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S6) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 6 session mean 時系列
        f.write("\n## 2. 6 session mean 時系列 (eval 5-run mean, t/s)\n")
        f.write("ub    | S1_Seval | S2_Sevalcross | S3_Seval3s | S4_Seval4s | S5_Seval5s | S6_Seval6s | range | mean_of_6 | σ_session\n")
        f.write("------|----------|---------------|------------|------------|------------|------------|-------|-----------|----------\n")
        ...
        for ub in UBS:
            ...
            if len(finite) >= 2:
                ...
                f.write(
                    f"{ub}  | {cells[0]} | {cells[1]} | {cells[2]} | {cells[3]} | {cells[4]} | {cells[5]} | "
        ...
        # 5. Welch t (prior 5-session pool vs cur S6)
        f.write("\n## 5. Prior 5-session pool (S1+S2+S3+S4+S5) vs 本 Phase (S6) Welch t\n")
        ...
        # 6. pooled 30-run
        f.write("\n## 6. Pooled 30-run (S1+S2+S3+S4+S5+S6) 統計\n")
        ...
        # 9. 6-session verdict summary
        f.write("\n## 9. 6-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        # 10. ub=1584 崩壊頻度カウント (本 Phase 新規) + Wilson 95% CI
        ...
        p_hat = k / n_total if n_total > 0 else 0.0
        # Wilson 95% CI (z=1.96)
        z = 1.96
        if n_total > 0:
            denom = 1 + z * z / n_total
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (6 session)\n")
        ...
        # 12. ピーク順序 6-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (6 session)\n")
        ...
        # 13. モード分類 (S1-S3 / S4-S5 / S6) 比較
        f.write("\n## 13. モード分類 (S1-S3 mode A / S4-S5 mode B / S6) の eval_mean 比較\n")
        f.write("ub    | mode_A_mean (S1-S3) | mode_B_mean (S4-S5) | S6_mean | closer_to | Δ_vs_A | Δ_vs_B\n")
        f.write("------|---------------------|---------------------|---------|-----------|--------|-------\n")
        for ub in UBS:
            mode_A = []
            mode_B = []
            ...
            sa = stats(mode_A)
            sb = stats(mode_B)
            sc = stats(cur_eval[ub])
            if sa is None or sb is None or sc is None:
                f.write(f"{ub}  | insufficient data\n")
                continue
            dA = sc["mean"] - sa["mean"]
            dB = sc["mean"] - sb["mean"]
            closer = "mode_A" if abs(dA) < abs(dB) else "mode_B"
            f.write(f"{ub}  | {sa['mean']:19.3f} | {sb['mean']:19.3f} | {sc['mean']:7.3f} | "
                    f"{closer:9s} | {dA:+.3f} | {dB:+.3f}\n")

        # 14. warmup1 ub=1584 absolute 値のモード判定
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S6）\n")
        w1_1584 = None
        ...
        if w1_1584 is not None and eval_1584 is not None:
            ...
            # モード帯判定 (S1-S3: warmup1 abs 15.51-15.78, S4-S5: 14.78-15.37)

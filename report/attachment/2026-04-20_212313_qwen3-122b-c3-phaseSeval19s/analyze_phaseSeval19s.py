#!/usr/bin/env python3
"""analyze_phaseSeval19s.py - Phase S-eval-19session 集計スクリプト

19 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval13s + phaseSeval14s + 本 Phase phaseSeval19s) を合算し、
ub 別 n=16 の session 間 σ, 18-session verdict, ピーク順序安定性、pooled 95-run 統計、
...
CUR_SESSION_LABEL = "S19_phaseSeval19s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval19s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval19s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval19s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-19session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S19) 5-run 統計（eval フェーズ）\n")
        ...
        # 5. Welch t (prior 18-session pool vs cur S19)
        f.write("\n## 5. Prior 18-session pool (S1..S19) vs 本 Phase (S19) Welch t\n")
        ...
        # 9. 18-session verdict summary
        f.write("\n## 9. 18-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (19 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (19 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (19 session)\n")
        ...
        # 12. ピーク順序 18-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (19 session)\n")

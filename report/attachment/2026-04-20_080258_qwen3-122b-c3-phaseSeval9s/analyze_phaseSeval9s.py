#!/usr/bin/env python3
"""analyze_phaseSeval9s.py - Phase S-eval-9session 集計スクリプト

9 session (phaseSeval + phaseSevalcross + phaseSeval3s + phaseSeval4s + phaseSeval5s + phaseSeval6s + phaseSeval7s + phaseSeval8s + 本 Phase phaseSeval9s) を合算し、
ub 別 n=9 の session 間 σ, 9-session verdict, ピーク順序安定性、pooled 45-run 統計、
ub=1584/1586/1664 崩壊頻度 (Wilson 95% CI)、ub 別時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 / S8 / S9 のモード分類比較を出力する。
"""
...
MODE_GROUPS = {
    ...
    "prev_S7": ["S7_phaseSeval7s"],
    "prev_S8": ["S8_phaseSeval8s"],
    "cur_S9": ["S9_phaseSeval9s"],
}
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval9s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval9s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval9s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-9session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S9) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 9 session mean 時系列
        f.write("\n## 2. 9 session mean 時系列 (eval 5-run mean, t/s)\n")
        f.write("ub    | S1_Seval | S2_Scross | S3_3s   | S4_4s   | S5_5s   | S6_6s   | S7_7s   | S8_8s   | S9_9s   | range | mean_of_9 | σ_session\n")
        f.write("------|----------|-----------|---------|---------|---------|---------|---------|---------|---------|-------|-----------|----------\n")
        ...
        for ub in UBS:
            ...
            if len(finite) >= 2:
                ...
                f.write(
                    f"{ub}  | {cells[0]} | {cells[1]} | {cells[2]} | {cells[3]} | {cells[4]} | {cells[5]} | {cells[6]} | {cells[7]} | {cells[8]} | "
        ...
        # 5. Welch t (prior 8-session pool vs cur S9)
        f.write("\n## 5. Prior 8-session pool (S1+S2+S3+S4+S5+S6+S7+S8) vs 本 Phase (S9) Welch t\n")
        ...
        # 6. pooled 45-run
        f.write("\n## 6. Pooled 45-run (S1+S2+S3+S4+S5+S6+S7+S8+S9) 統計\n")
        ...
        # 9. 9-session verdict summary
        f.write("\n## 9. 9-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (9 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (9 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (9 session)\n")
        ...
        # 12. ピーク順序 9-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (9 session)\n")
        ...
        # 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S8 D / S7 / S9) 比較
        f.write("\n## 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S8 D / S7 / S9) の eval_mean 比較\n")
        f.write("ub    | modeA_mean | modeB_mean | modeC_mean | modeD_mean | S7_mean | S8_mean | S9_mean | closer_to | dA     | dB     | dC     | dD     | dS7    | dS8\n")
        f.write("------|------------|------------|------------|------------|---------|---------|---------|-----------|--------|--------|--------|--------|--------|--------\n")
        for ub in UBS:
            mode_A = []
            mode_B = []
            ...
            for lbl in MODE_GROUPS["prev_S7"]:
                prev_S7_list.extend(session_data.get(lbl, {}).get(ub, []))
            for lbl in MODE_GROUPS["prev_S8"]:
                prev_S8_list.extend(session_data.get(lbl, {}).get(ub, []))
            sa = stats(mode_A)
            sb = stats(mode_B)
            sc = stats(mode_C)
            sd = stats(mode_D)
            s7 = stats(prev_S7_list)
            s8 = stats(prev_S8_list)
            s9 = stats(cur_eval[ub])
            if sa is None or sb is None or sc is None or sd is None or s7 is None or s8 is None or s9 is None:
                f.write(f"{ub}  | insufficient data\n")
                continue
            dA = s9["mean"] - sa["mean"]
            dB = s9["mean"] - sb["mean"]
            ...
            dS7 = s9["mean"] - s7["mean"]
            dS8 = s9["mean"] - s8["mean"]
            d_abs = {"mode_A": abs(dA), "mode_B": abs(dB), "mode_C": abs(dC), "mode_D": abs(dD), "S7": abs(dS7), "S8": abs(dS8)}
            closer = min(d_abs, key=d_abs.get)
            f.write(f"{ub}  | {sa['mean']:10.3f} | {sb['mean']:10.3f} | {sc['mean']:10.3f} | {sd['mean']:10.3f} | {s7['mean']:7.3f} | {s8['mean']:7.3f} | {s9['mean']:7.3f} | "
                    f"{closer:9s} | {dA:+.3f} | {dB:+.3f} | {dC:+.3f} | {dD:+.3f} | {dS7:+.3f} | {dS8:+.3f}\n")

        # 14. warmup1 ub=1584 absolute 値のモード判定
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S9）\n")
        w1_1584 = None

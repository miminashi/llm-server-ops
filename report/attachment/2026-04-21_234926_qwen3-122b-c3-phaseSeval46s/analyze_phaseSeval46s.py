#!/usr/bin/env python3
"""analyze_phaseSeval46s.py - Phase S-eval-46session 集計スクリプト

46 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval45s + 本 Phase phaseSeval46s) を合算し、
ub 別 n=46 の session 間 σ, 46-session verdict, ピーク順序安定性、pooled 230-run 統計、
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval46s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval46s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval46s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-46session verdict\n")
        ...
        # 1. 本 Phase (S46) 5-run 統計
        f.write("## 1. 本 Phase (S46) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 46 session mean 時系列
        f.write("\n## 2. 46 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 45-session pool vs cur S46)
        f.write("\n## 5. Prior 45-session pool (S1..S45) vs 本 Phase (S46) Welch t\n")
        ...
        # 9. 46-session verdict summary
        f.write("\n## 9. 46-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (46 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (46 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (46 session)\n")
        ...
        # 12. ピーク順序 46-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (46 session)\n")

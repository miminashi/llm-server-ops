#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 46 session + Sbfine 3 reference points の時系列プロット
...
- S1..S46 (5-run mean) を x=1..46 に折れ線 + 丸 marker でプロット
...
# S1..S45 summary TSV path (phase=eval のみ抽出) + S46 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S46", None, "summary_phaseSeval46s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval46s.tsv
]
...
def main() -> int:
    ...
    ax.set_title(
        "Phase S-eval 46-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

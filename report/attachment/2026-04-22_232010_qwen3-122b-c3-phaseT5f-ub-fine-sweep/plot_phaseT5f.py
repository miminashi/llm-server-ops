#!/usr/bin/env python3
"""plot_phaseT5f.py - Phase T-5f: B28 × ctx=32k × ub 微細スイープ グラフ生成

3 枚の PNG を出力:
  1. phaseT5f_ub_trend.png: ub (log scale) vs eval_mean + prompt_mean dual y-axis line
  2. phaseT5f_pareto.png:   x=prompt_mean, y=eval_mean scatter (ub ラベル付き)
  3. phaseT5f_drift.png:    run_index=1..9 vs eval_mean、起点・終点強調
"""
...
RUN_ORDER = [
    ("B28_32k_ub512a", 512, "drift start"),
    ("B28_32k_ub1586", 1586, ""),
    ("B28_32k_ub1024", 1024, ""),
    ("B28_32k_ub768",  768, ""),
    ("B28_32k_ub384",  384, ""),
    ...
    ("B28_32k_ub128",  128, ""),
    ("B28_32k_ub64",    64, ""),
    ("B28_32k_ub512z", 512, "drift end"),
]


def load_stats():
    out = {}
    with (SCRIPT_DIR / "phaseT5f_stats.csv").open() as f:
...
def plot_ub_trend(data):
    ...
    ax1.axhline(y=PEAK_PHASE_T5E_BEST, color="#8B0000", ls="--", alpha=0.7,
                label=f"T-5e ({PEAK_PHASE_T5E_BEST})")
    ax1.axhline(y=PEAK_PHASE_T5_BEST, color="#DD8452", ls="--", alpha=0.6,
                label=f"T-5 ({PEAK_PHASE_T5_BEST})")
    ax1.axhline(y=PEAK_PHASE_S, color="#8172B2", ls="--", alpha=0.5,
                label=f"S ({PEAK_PHASE_S})")
    ...
    fig.suptitle("Phase T-5f: B28 × ctx=32k × ub trend (eval + prompt dual-axis)",
                 fontsize=12)
    fig.tight_layout()
    out = SCRIPT_DIR / "phaseT5f_ub_trend.png"
...
def plot_drift(data):
    ...
    if len(xs) >= 9:
        ...
        # drift 補正後トレンド
        per_run = (ys[-1] - ys[0]) / (xs[-1] - xs[0])
        corr = [y - per_run * (x - xs[0]) for x, y in zip(xs, ys)]
        ax.plot(xs, corr, "g:.", lw=1.5, alpha=0.75,
                label=f"drift-corrected (per_run={per_run:+.4f})")

    ax.axhline(y=PEAK_PHASE_T5E_BEST, color="#8B0000", ls="--", alpha=0.7,
               label=f"T-5e ({PEAK_PHASE_T5E_BEST})")
    ax.axhline(y=PEAK_PHASE_T5_BEST, color="#DD8452", ls="--", alpha=0.5,
               label=f"T-5 ({PEAK_PHASE_T5_BEST})")
    ax.set_xlabel("run_index (batch 実行順)")
    ax.set_ylabel("eval_tps (t/s)")
    ax.set_title("Phase T-5f: session drift bracket (run_index 順)")
    ax.set_xticks(xs)
    ax.grid(True, alpha=0.3)
    ax.legend(loc="lower left", fontsize=8)

    fig.tight_layout()
    out = SCRIPT_DIR / "phaseT5f_drift.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"[plot] wrote {out}")
    plt.close(fig)

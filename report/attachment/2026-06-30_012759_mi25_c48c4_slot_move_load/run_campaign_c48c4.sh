#!/bin/bash
# mi25 c48c4 (SLOT8 = BDF 84:00.0) 単独 stand-alone 8h 負荷キャンペーン オーケストレータ。
# run_campaign_8820.sh (8820=c48c4 が SLOT6=BDF 87:00.0 にあったときの試験) から派生。
# - c48c4 を SLOT6→SLOT8 に物理移動、Vulkan idx 3→2 (post-swap で c48c4 が GPU[2])
# - 24h cap → 8h cap、MIN/MAX/HANG_SAFETY を 8h 短時間運用に調整
# - restart_llama を c48c4-slot8 単独可視化ラッパに差し替え
# - BACO 後 power cap 160W 強制再設定ブロックを restart_llama 後に追加 (ssh 経由 sudo、user 許可済)
#
# 試行ループを回し、ハング時は KVMスクショ→BMCリセット→復帰→ブート状態記録→ロック再取得
# →llama再起動→テレメトリ再起動 を自動化して継続する。
#
# Usage:
#   run_campaign_c48c4.sh
#   主要パラメータは環境変数で上書き可:
#   MAX_TRIALS(60) MIN_TRIALS(20) HANG_SAFETY(5) PHASE_CAP_SEC(28800=8h) TRIAL_SEC(720)
#   CTX_SIZE(131072 = run-c48c4-slot8.sh 経由でラッパに渡す)
#
# 終了コード: 0=正常完了 / 7=ハング安全境界に到達(ユーザ確認要) / 9=復旧失敗(要介入)

set -u
PROJ=/home/ubuntu/projects/llm-server-ops
SCRATCH=/home/ubuntu/projects/llm-server-ops/report/attachment/2026-06-30_012759_mi25_c48c4_slot_move_load
SERVER=mi25
ENDPOINT=http://10.1.4.13:8000
MODEL="unsloth/Qwen3-8B-GGUF:Q6_K"
BACKEND="vulkan"
ROCM_DEVICE_IDX=2  # post-swap で c48c4 が GPU[2] (BDF 84:00.0)

MAX_TRIALS="${MAX_TRIALS:-60}"
MIN_TRIALS="${MIN_TRIALS:-20}"
HANG_SAFETY="${HANG_SAFETY:-5}"
PHASE_CAP_SEC="${PHASE_CAP_SEC:-28800}"
...
restart_llama(){
  log "llama 再起動 (c48c4 SLOT8 stand-alone Vulkan ctx=$CTX_SIZE)"
  # c48c4 単独可視化ラッパを呼ぶ (start.sh の detect_radv_vk_indices を回避)
  CTX_SIZE="$CTX_SIZE" bash "$SCRATCH/run-c48c4-slot8.sh" >> "$CAMPLOG" 2>&1
  local rc=$?
...
recover_from_hang(){ # $1=hang index。成功:0 / 失敗:9
  ...
  ( cd "$PROJ" && .claude/skills/gpu-server/scripts/unlock.sh "$SERVER"; .claude/skills/gpu-server/scripts/lock.sh "$SERVER" "c48c4-slot-move-2026-06-30_012759" ) >> "$CAMPLOG" 2>&1
  # 5) llama 再起動
  restart_llama
  # 6) /health 確認
...
record_boot_state "phase-start-${BACKEND}-c48c4-slot8"
start_telemetry

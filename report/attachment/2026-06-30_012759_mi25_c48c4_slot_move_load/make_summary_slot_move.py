#!/usr/bin/env python3
"""mi25 c48c4 SLOT8 移動 + Vulkan 8h 負荷 — 集計 + summary.png。
make_summary_standalone.py (元 8820 SLOT6 24h) から派生。差分:
- 1 round 構成 (round1+round2 は廃止)
- GPU[3] → GPU[2] (post-swap で c48c4 が GPU[2])
- 過去 2 種類との Fisher 比較 (4 枚運用 88/3 と stand_alone_24h SLOT6 147/2)
"""
import json
import math
import os
...
def main():
    ...
    baseline_file = os.path.join(SCR, "kern_dmesg_baseline_t1.log")
    baseline_lines = 0
    ...
    md = []
    md.append("# mi25 c48c4 SLOT8 移動 + Vulkan 8h 負荷 — 集計表\n\n")
    md.append("## 全期間サマリ\n")
    md.append("| 項目 | 値 |\n|---|---|\n")
    ...
    md.append(f"## GPU[{TARGET_GPU_IDX}] (c48c4) テレメトリ\n")
    if r:
    ...
    title = (
        f"mi25 c48c4 SLOT8 / Vulkan stand-alone {duration_h:.1f}h — "

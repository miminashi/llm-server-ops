#!/usr/bin/env python3
"""analyze_phaseSeval33s.py - Phase S-eval-33session 集計スクリプト

33 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval32s + 本 Phase phaseSeval33s) を合算し、
ub 別 n=33 の session 間 σ, 33-session verdict, ピーク順序安定性、pooled 165-run 統計、
...
CUR_SESSION_LABEL = "S33_phaseSeval33s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval33s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval33s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval33s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-33session verdict\n")
        ...
        # 1. 本 Phase (S33) 5-run 統計
        f.write("## 1. 本 Phase (S33) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 33 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 32-session pool vs cur S33)
        f.write("\n## 5. Prior 32-session pool (S1..S32) vs 本 Phase (S33) Welch t\n")
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (33 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (33 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (33 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (33 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S33）\n")
        w1_1584 = None

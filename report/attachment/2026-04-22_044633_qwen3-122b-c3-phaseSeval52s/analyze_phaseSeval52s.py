#!/usr/bin/env python3
"""analyze_phaseSeval52s.py - Phase S-eval-52session 集計スクリプト

52 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval51s + 本 Phase phaseSeval52s) を合算し、
...
CUR_SESSION_LABEL = "S52_phaseSeval52s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval52s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval52s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval52s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-52session verdict\n")
        ...
        f.write("\n## 5. Prior 51-session pool (S1..S51) vs 本 Phase (S52) Welch t\n")

#!/usr/bin/env python3
"""analyze_phaseSeval28s.py - Phase S-eval-28session 集計スクリプト

28 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval27s + 本 Phase phaseSeval28s) を合算し、
ub 別 n=28 の session 間 σ, 28-session verdict, ピーク順序安定性、pooled 140-run 統計、
...
CUR_SESSION_LABEL = "S28_phaseSeval28s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval28s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval28s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval28s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-28session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S28) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 28 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 27-session pool vs cur S28)
        f.write("\n## 5. Prior 27-session pool (S1..S27) vs 本 Phase (S28) Welch t\n")
        ...
        f.write("\n## 9. 28-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (28 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (28 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (28 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (28 session)\n")

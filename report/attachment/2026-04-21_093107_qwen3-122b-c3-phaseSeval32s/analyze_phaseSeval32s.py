#!/usr/bin/env python3
"""analyze_phaseSeval32s.py - Phase S-eval-32session 集計スクリプト

32 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval31s + 本 Phase phaseSeval32s) を合算し、
ub 別 n=32 の session 間 σ, 32-session verdict, ピーク順序安定性、pooled 160-run 統計、
...
CUR_SESSION_LABEL = "S32_phaseSeval32s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval32s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval32s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval32s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-32session verdict\n")
        ...
        # 1. 本 Phase (S32) 5-run 統計
        f.write("## 1. 本 Phase (S32) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 32 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 31-session pool vs cur S32)
        f.write("\n## 5. Prior 31-session pool (S1..S31) vs 本 Phase (S32) Welch t\n")
        ...
        f.write("\n## 9. 31-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (32 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (32 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (32 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (32 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S32）\n")
        w1_1584 = None

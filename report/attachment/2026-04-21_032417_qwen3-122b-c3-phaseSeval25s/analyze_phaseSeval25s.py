#!/usr/bin/env python3
"""analyze_phaseSeval25s.py - Phase S-eval-25session 集計スクリプト

25 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval24s + 本 Phase phaseSeval25s) を合算し、
ub 別 n=25 の session 間 σ, 25-session verdict, ピーク順序安定性、pooled 125-run 統計、
...
CUR_SESSION_LABEL = "S25_phaseSeval25s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval25s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval25s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval25s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-25session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S25) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 25 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 24-session pool vs cur S25)
        f.write("\n## 5. Prior 24-session pool (S1..S24) vs 本 Phase (S25) Welch t\n")
        ...
        f.write("\n## 9. 25-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (25 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (25 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (25 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (25 session)\n")

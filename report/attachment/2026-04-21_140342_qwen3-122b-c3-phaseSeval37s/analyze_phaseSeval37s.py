#!/usr/bin/env python3
"""analyze_phaseSeval37s.py - Phase S-eval-37session 集計スクリプト

37 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval36s + 本 Phase phaseSeval37s) を合算し、
ub 別 n=37 の session 間 σ, 37-session verdict, ピーク順序安定性、pooled 185-run 統計、
...
CUR_SESSION_LABEL = "S37_phaseSeval37s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval37s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval37s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval37s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-37session verdict\n")
        ...
        # 1. 本 Phase (S37) 5-run 統計
        f.write("## 1. 本 Phase (S37) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 37 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 36-session pool vs cur S37)
        f.write("\n## 5. Prior 36-session pool (S1..S36) vs 本 Phase (S37) Welch t\n")
        ...
        # 9. 37-session verdict summary
        f.write("\n## 9. 37-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (37 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (37 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (37 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (37 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S37）\n")
        w1_1584 = None

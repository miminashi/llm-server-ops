...
if pp:
    # 形式: {"tag": ..., "pp512": {...}, "pp2048": {...}, ...}
    keys = [k for k in pp[modes[0]] if k.startswith('pp')]
    print("| サイズ | " + " | ".join(modes) + " |")
    print("|---|" + "---:|" * len(modes))
    for k in keys:
        cells = [f"{pp[m][k]['prompt_per_second']:.1f}" for m in modes]
        print(f"| {k} | " + " | ".join(cells) + " |")
    print("\n## depth-0 decode (n=64, t/s)")
    for k in keys:
        print(f"| {k} | " + " | ".join(f"{pp[m][k]['predicted_per_second']:.2f}" for m in modes) + " |")

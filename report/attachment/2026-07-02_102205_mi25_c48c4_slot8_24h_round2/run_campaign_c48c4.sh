...
SCRATCH=/home/ubuntu/projects/llm-server-ops/report/attachment/2026-07-02_102205_mi25_c48c4_slot8_24h_round2
SERVER=mi25
ENDPOINT=http://10.1.4.13:8000
MODEL="unsloth/Qwen3-8B-GGUF:Q6_K"
BACKEND="vulkan"
ROCM_DEVICE_IDX=2  # c48c4 = GPU[2] (BDF 84:00.0 = SLOT8、c48c4 slot move 試験後の配置維持)
...
recover_from_hang(){ # $1=hang index。成功:0 / 失敗:9
  ...
  ( cd "$PROJ" && .claude/skills/gpu-server/scripts/unlock.sh "$SERVER"; .claude/skills/gpu-server/scripts/lock.sh "$SERVER" "c48c4-slot8-24h-r2-2026-07-02_102205" ) >> "$CAMPLOG" 2>&1
  # 5) llama 再起動
  restart_llama
  # 6) /health 確認

#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 40 session + Sbfine 3 reference points の時系列プロット
...
- S1..S40 (5-run mean) を x=1..40 に折れ線 + 丸 marker でプロット
...
# S1..S39 summary TSV path (phase=eval のみ抽出) + S40 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S40", None, "summary_phaseSeval40s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval40s.tsv
]
...
def main() -> int:
    ...
    ax.set_title(
        "Phase S-eval 40-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

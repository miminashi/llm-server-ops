#!/usr/bin/env python3
"""analyze_phaseSeval40s.py - Phase S-eval-40session 集計スクリプト

40 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval39s + 本 Phase phaseSeval40s) を合算し、
ub 別 n=40 の session 間 σ, 40-session verdict, ピーク順序安定性、pooled 200-run 統計、
...
CUR_SESSION_LABEL = "S40_phaseSeval40s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval40s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval40s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval40s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-40session verdict\n")
        ...
        # 1. 本 Phase (S40) 5-run 統計
        f.write("## 1. 本 Phase (S40) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 40 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 39-session pool vs cur S40)
        f.write("\n## 5. Prior 39-session pool (S1..S39) vs 本 Phase (S40) Welch t\n")
        ...
        # 9. 40-session verdict summary
        f.write("\n## 9. 40-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (40 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (40 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (40 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (40 session)\n")

#!/usr/bin/env python3
"""analyze_phaseSeval26s.py - Phase S-eval-26session 集計スクリプト

26 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval25s + 本 Phase phaseSeval26s) を合算し、
ub 別 n=26 の session 間 σ, 26-session verdict, ピーク順序安定性、pooled 130-run 統計、
...
CUR_SESSION_LABEL = "S26_phaseSeval26s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval26s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval26s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval26s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-26session verdict\n")
        ...
        f.write("\n## 2. 26 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 25-session pool vs cur S26)
        f.write("\n## 5. Prior 25-session pool (S1..S25) vs 本 Phase (S26) Welch t\n")
        ...
        f.write("\n## 9. 26-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (26 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (26 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (26 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (26 session)\n")

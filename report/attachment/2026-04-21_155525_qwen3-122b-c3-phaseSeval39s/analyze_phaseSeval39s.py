#!/usr/bin/env python3
"""analyze_phaseSeval39s.py - Phase S-eval-39session 集計スクリプト

39 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval38s + 本 Phase phaseSeval39s) を合算し、
ub 別 n=39 の session 間 σ, 39-session verdict, ピーク順序安定性、pooled 195-run 統計、
...
CUR_SESSION_LABEL = "S39_phaseSeval39s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval39s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval39s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval39s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-39session verdict\n")
        ...
        # 1. 本 Phase (S39) 5-run 統計
        f.write("## 1. 本 Phase (S39) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 39 session mean 時系列
        f.write("\n## 2. 39 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 38-session pool vs cur S39)
        f.write("\n## 5. Prior 38-session pool (S1..S38) vs 本 Phase (S39) Welch t\n")
        ...
        # 9. 39-session verdict summary
        f.write("\n## 9. 39-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (39 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (39 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (39 session)\n")
        ...
        # 12. ピーク順序 39-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (39 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S39）\n")
        w1_1584 = None

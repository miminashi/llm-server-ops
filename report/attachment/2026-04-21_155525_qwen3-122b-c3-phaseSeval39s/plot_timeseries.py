#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 39 session + Sbfine 3 reference points の時系列プロット
...
- S1..S39 (5-run mean) を x=1..39 に折れ線 + 丸 marker でプロット
...
S_EVAL_DIRS = [
    ...
    ("S39", None, "summary_phaseSeval39s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval39s.tsv
]
...
def main() -> int:
    ...
    ax.set_title(
        "Phase S-eval 39-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

#!/usr/bin/env bash
# start_phaseR.sh - Phase R-ctx3: fa=1 中間 ctx (32768/65536) 起動試験
# Phase R からの変更点:
#   - REMOTE_LOG プレフィックスを phaseR_ → phaseRctx3_ に変更（Phase R ログとの衝突防止）
# 起動タイムアウト 300s、OOM 検知パターン、-ub 下限拒否は Phase R と同一
...
REMOTE_LOG="/tmp/llama-server_phaseRctx3_fa${FLASH_ATTN}_ctx${CTX_SIZE}_b${BATCH_SIZE}_ub${UB_SIZE}.log"

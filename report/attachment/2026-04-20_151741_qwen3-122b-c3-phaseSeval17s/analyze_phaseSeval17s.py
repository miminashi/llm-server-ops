#!/usr/bin/env python3
"""analyze_phaseSeval17s.py - Phase S-eval-17session 集計スクリプト

17 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval13s + phaseSeval14s + 本 Phase phaseSeval17s) を合算し、
ub 別 n=16 の session 間 σ, 17-session verdict, ピーク順序安定性、pooled 85-run 統計、
...
CUR_SESSION_LABEL = "S17_phaseSeval17s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval17s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval17s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval17s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-17session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S17) 5-run 統計（eval フェーズ）\n")
        ...
        # 5. Welch t (prior 16-session pool vs cur S17)
        f.write("\n## 5. Prior 16-session pool (S1..S16) vs 本 Phase (S17) Welch t\n")
        ...
        # 9. 17-session verdict summary
        f.write("\n## 9. 17-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (17 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (17 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (17 session)\n")
        ...
        # 12. ピーク順序 17-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (17 session)\n")

#!/usr/bin/env python3
"""analyze_phaseSeval11s.py - Phase S-eval-11session 集計スクリプト

11 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval10s + 本 Phase phaseSeval11s) を合算し、
ub 別 n=11 の session 間 σ, 11-session verdict, ピーク順序安定性、pooled 55-run 統計、
ub=1584/1586/1664 崩壊頻度 (Wilson 95% CI)、ub 別時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 / S8 / S9 / S10 / S11 のモード分類比較を出力する。
"""
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval11s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval11s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval11s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-11session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S11) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 11 session mean 時系列
        f.write("\n## 2. 11 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 10-session pool vs cur S11)
        f.write("\n## 5. Prior 10-session pool (S1..S10) vs 本 Phase (S11) Welch t\n")
        ...
        # 6. pooled 55-run
        f.write("\n## 6. Pooled 55-run (S1+S2+S3+S4+S5+S6+S7+S8+S9+S10+S11) 統計\n")
        ...
        # 9. 11-session verdict summary
        f.write("\n## 9. 11-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (11 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (11 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (11 session)\n")
        ...
        # 12. ピーク順序 11-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (11 session)\n")
        ...
        # 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S8 D / S9 / S10 / S11) 比較
        f.write("\n## 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S8 D / S9 / S10 / S11) の eval_mean 比較\n")
        f.write("ub    | modeA_mean | modeB_mean | modeC_mean | modeD_mean | S9_mean | S10_mean | S11_mean | closer_to | dA     | dB     | dC     | dD     | dS9    | dS10\n")
        f.write("------|------------|------------|------------|------------|---------|----------|----------|-----------|--------|--------|--------|--------|--------|--------\n")
        for ub in UBS:
            mode_A = []
            mode_B = []
            ...
            s11 = stats(cur_eval[ub])
            if sa is None or sb is None or sc is None or sd is None or s9 is None or s10 is None or s11 is None:
                f.write(f"{ub}  | insufficient data\n")
                continue
            dA = s11["mean"] - sa["mean"]
            dB = s11["mean"] - sb["mean"]
            dC = s11["mean"] - sc["mean"]
            dD = s11["mean"] - sd["mean"]
            dS9 = s11["mean"] - s9["mean"]
            dS10 = s11["mean"] - s10["mean"]
            d_abs = {"mode_A": abs(dA), "mode_B": abs(dB), "mode_C": abs(dC), "mode_D": abs(dD), "S9": abs(dS9), "S10": abs(dS10)}
            closer = min(d_abs, key=d_abs.get)
            f.write(f"{ub}  | {sa['mean']:10.3f} | {sb['mean']:10.3f} | {sc['mean']:10.3f} | {sd['mean']:10.3f} | {s9['mean']:7.3f} | {s10['mean']:8.3f} | {s11['mean']:8.3f} | "
                    f"{closer:9s} | {dA:+.3f} | {dB:+.3f} | {dC:+.3f} | {dD:+.3f} | {dS9:+.3f} | {dS10:+.3f}\n")

        # 14. warmup1 ub=1584 absolute 値のモード判定
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S11）\n")
        w1_1584 = None

#!/usr/bin/env python3
"""analyze_phaseSeval50s.py - Phase S-eval-50session 集計スクリプト

50 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval49s + 本 Phase phaseSeval50s) を合算し、
ub 別 n=50 の session 間 σ, 50-session verdict, ピーク順序安定性、pooled 250-run 統計、
...
CUR_SESSION_LABEL = "S50_phaseSeval50s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval50s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval50s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval50s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-50session verdict\n")
        ...
        # 1. 本 Phase (S50) 5-run 統計
        f.write("## 1. 本 Phase (S50) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 50 session mean 時系列
        f.write("\n## 2. 50 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 49-session pool vs cur S50)
        f.write("\n## 5. Prior 49-session pool (S1..S49) vs 本 Phase (S50) Welch t\n")
        ...
        # 9. 50-session verdict summary
        f.write("\n## 9. 50-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (50 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (50 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (50 session)\n")
        ...
        # 12. ピーク順序 50-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (50 session)\n")

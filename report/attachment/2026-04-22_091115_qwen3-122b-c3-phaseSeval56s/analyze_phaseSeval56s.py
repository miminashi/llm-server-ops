#!/usr/bin/env python3
"""analyze_phaseSeval56s.py - Phase S-eval-56session 集計スクリプト

55 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval55s + 本 Phase phaseSeval56s) を合算し、
ub 別 n=56 の session 間 σ, 56-session verdict, ピーク順序安定性、pooled 280-run 統計、
...
CUR_SESSION_LABEL = "S56_phaseSeval56s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval56s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval56s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval56s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-55session verdict\n")

#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 56 session + Sbfine 3 reference points の時系列プロット
...
- S1..S56 (5-run mean) を x=1..56 に折れ線 + 丸 marker でプロット
- 崩壊閾値 eval_mean=15.0 を水平線で表示
- S1..S56 に対する線形回帰直線（trend line）を各 ub ごとに dashed で重ねる
"""
...
# S1..S56 summary TSV path (phase=eval のみ抽出) + S56 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S56", None, "summary_phaseSeval56s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval56s.tsv
]
...
def main() -> int:
    ...
    for ub in UBS:
        ...
        # 回帰直線 (S1..S56 only、Sbfine ref は除外)
    ...
    ax.set_title(
        "Phase S-eval 55-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

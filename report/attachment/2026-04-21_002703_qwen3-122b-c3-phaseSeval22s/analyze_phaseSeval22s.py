#!/usr/bin/env python3
"""analyze_phaseSeval22s.py - Phase S-eval-22session 集計スクリプト

22 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval21s + 本 Phase phaseSeval22s) を合算し、
ub 別 n=22 の session 間 σ, 22-session verdict, ピーク順序安定性、pooled 110-run 統計、
...
CUR_SESSION_LABEL = "S22_phaseSeval22s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval22s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval22s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval22s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-22session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S22) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 5. Prior 21-session pool (S1..S21) vs 本 Phase (S22) Welch t\n")
        ...
        f.write("\n## 9. 22-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (22 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (22 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (22 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (22 session)\n")

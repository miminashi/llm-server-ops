#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 38 session + Sbfine 3 reference points の時系列プロット
...
- S1..S38 (5-run mean) を x=1..38 に折れ線 + 丸 marker でプロット
...
# S1..S38 summary TSV path (phase=eval のみ抽出)
S_EVAL_DIRS = [
    ...
    ("S38", None, "summary_phaseSeval38s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval38s.tsv
]
...
def main() -> int:
    ...
    ax.set_title(
        "Phase S-eval 38-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

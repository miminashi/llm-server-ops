#!/usr/bin/env python3
"""analyze_phaseSeval38s.py - Phase S-eval-38session 集計スクリプト

38 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval37s + 本 Phase phaseSeval38s) を合算し、
ub 別 n=38 の session 間 σ, 38-session verdict, ピーク順序安定性、pooled 190-run 統計、
...
CUR_SESSION_LABEL = "S38_phaseSeval38s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval38s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval38s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval38s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-38session verdict\n")
        ...
        # 1. 本 Phase (S38) 5-run 統計
        f.write("## 1. 本 Phase (S38) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 38 session mean 時系列
        f.write("\n## 2. 38 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 37-session pool vs cur S38)
        f.write("\n## 5. Prior 37-session pool (S1..S37) vs 本 Phase (S38) Welch t\n")
        ...
        # 9. 38-session verdict summary
        f.write("\n## 9. 38-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (38 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (38 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (38 session)\n")
        ...
        # 12. ピーク順序 38-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (38 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S38）\n")
        w1_1584 = None

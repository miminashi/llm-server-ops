#!/usr/bin/env python3
"""analyze_phaseSeval36s.py - Phase S-eval-36session 集計スクリプト

36 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval35s + 本 Phase phaseSeval36s) を合算し、
ub 別 n=36 の session 間 σ, 36-session verdict, ピーク順序安定性、pooled 180-run 統計、
...
CUR_SESSION_LABEL = "S36_phaseSeval36s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval36s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval36s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval36s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-36session verdict\n")
        ...
        # 1. 本 Phase (S36) 5-run 統計
        f.write("## 1. 本 Phase (S36) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 36 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 35-session pool vs cur S36)
        f.write("\n## 5. Prior 35-session pool (S1..S35) vs 本 Phase (S36) Welch t\n")
        ...
        # 9. 36-session verdict summary
        f.write("\n## 9. 36-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (36 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (36 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (36 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (36 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S36）\n")
        w1_1584 = None

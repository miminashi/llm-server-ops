#!/usr/bin/env python3
"""analyze_phaseSeval27s.py - Phase S-eval-27session 集計スクリプト

27 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval26s + 本 Phase phaseSeval27s) を合算し、
ub 別 n=27 の session 間 σ, 27-session verdict, ピーク順序安定性、pooled 135-run 統計、
...
CUR_SESSION_LABEL = "S27_phaseSeval27s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval27s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval27s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval27s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-27session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S27) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 27 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 26-session pool vs cur S27)
        f.write("\n## 5. Prior 26-session pool (S1..S26) vs 本 Phase (S27) Welch t\n")
        ...
        f.write("\n## 9. 27-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (27 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (27 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (27 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (27 session)\n")

#!/usr/bin/env python3
"""analyze_phaseSeval8s.py - Phase S-eval-8session 集計スクリプト

8 session (phaseSeval + phaseSevalcross + phaseSeval3s + phaseSeval4s + phaseSeval5s + phaseSeval6s + phaseSeval7s + 本 Phase phaseSeval8s) を合算し、
ub 別 n=8 の session 間 σ, 8-session verdict, ピーク順序安定性、pooled 40-run 統計、
ub=1584/1586/1664 崩壊頻度 (Wilson 95% CI)、ub 別時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 / S8 のモード分類比較を出力する。
"""
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval8s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval8s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval8s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-8session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S8) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 8 session mean 時系列
        f.write("\n## 2. 8 session mean 時系列 (eval 5-run mean, t/s)\n")
        f.write("ub    | S1_Seval | S2_Scross | S3_3s   | S4_4s   | S5_5s   | S6_6s   | S7_7s   | S8_8s   | range | mean_of_8 | σ_session\n")
        f.write("------|----------|-----------|---------|---------|---------|---------|---------|---------|-------|-----------|----------\n")
        ...
        for ub in UBS:
            ...
            if len(finite) >= 2:
                ...
                f.write(
                    f"{ub}  | {cells[0]} | {cells[1]} | {cells[2]} | {cells[3]} | {cells[4]} | {cells[5]} | {cells[6]} | {cells[7]} | "
        ...
        # 5. Welch t (prior 7-session pool vs cur S8)
        f.write("\n## 5. Prior 7-session pool (S1+S2+S3+S4+S5+S6+S7) vs 本 Phase (S8) Welch t\n")
        ...
        # 6. pooled 40-run
        f.write("\n## 6. Pooled 40-run (S1+S2+S3+S4+S5+S6+S7+S8) 統計\n")
        ...
        # 9. 8-session verdict summary
        f.write("\n## 9. 8-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (8 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (8 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (8 session)\n")
        ...
        # 12. ピーク順序 8-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (8 session)\n")
        ...
        # 13. モード分類 (S1-S3 / S4-S5 / S6 / S7 / S8) 比較
        f.write("\n## 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S7 / S8) の eval_mean 比較\n")
        f.write("ub    | modeA_mean | modeB_mean | modeC_mean | S7_mean | S8_mean | closer_to | dA     | dB     | dC     | dS7\n")
        f.write("------|------------|------------|------------|---------|---------|-----------|--------|--------|--------|--------\n")
        for ub in UBS:
            mode_A = []
            mode_B = []
            ...
            s8 = stats(cur_eval[ub])
            if sa is None or sb is None or sc is None or s7 is None or s8 is None:
                f.write(f"{ub}  | insufficient data\n")
                continue
            dA = s8["mean"] - sa["mean"]
            dB = s8["mean"] - sb["mean"]
            dC = s8["mean"] - sc["mean"]
            dS7 = s8["mean"] - s7["mean"]
            d_abs = {"mode_A": abs(dA), "mode_B": abs(dB), "mode_C": abs(dC), "S7": abs(dS7)}
            closer = min(d_abs, key=d_abs.get)
            f.write(f"{ub}  | {sa['mean']:10.3f} | {sb['mean']:10.3f} | {sc['mean']:10.3f} | {s7['mean']:7.3f} | {s8['mean']:7.3f} | "
                    f"{closer:9s} | {dA:+.3f} | {dB:+.3f} | {dC:+.3f} | {dS7:+.3f}\n")

        # 14. warmup1 ub=1584 absolute 値のモード判定
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S8）\n")
        w1_1584 = None

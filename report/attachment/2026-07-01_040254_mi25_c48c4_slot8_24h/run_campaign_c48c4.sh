...
SCRATCH=/home/ubuntu/projects/llm-server-ops/report/attachment/2026-07-01_040254_mi25_c48c4_slot8_24h
SERVER=mi25
...
recover_from_hang(){ # $1=hang index。成功:0 / 失敗:9
  ...
  ( cd "$PROJ" && .claude/skills/gpu-server/scripts/unlock.sh "$SERVER"; .claude/skills/gpu-server/scripts/lock.sh "$SERVER" "c48c4-slot8-24h-2026-07-01_040254" ) >> "$CAMPLOG" 2>&1
  # 5) llama 再起動
  restart_llama
  # 6) /health 確認

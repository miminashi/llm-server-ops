#!/bin/bash
# c48c4 単独可視化 + Vulkan + Qwen3-8B Q6_K で llama-server を起動するスタンドアロン起動コマンド。
# 元 run-8820-stand-alone.sh の派生 (2026-06-30: c48c4 が SLOT8 へ移動、Vulkan idx は 2 に変更)。
...
# 設定値は start.sh の mi25 + vulkan 経路を複製。

set -euo pipefail
SERVER=mi25

# c48c4 = SLOT8 (BDF 84:00.0) = GPU[2] = Vulkan idx 2 のみ可視化 (multi-GPU 経路を排除)
GGML_VK_IDX="2"
...
echo "==> llama-server 起動 (c48c4 SLOT8 stand-alone Vulkan ctx=$CTX_SIZE)"

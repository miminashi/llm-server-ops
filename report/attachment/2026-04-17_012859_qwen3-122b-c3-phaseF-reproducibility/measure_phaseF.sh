#!/usr/bin/env bash
# measure_phaseF.sh - Phase F 計測（dmon + curl eval + status + numastat snap）
# usage: measure_phaseF.sh <pid> <tag>
...
log "==== measure_phaseF start tag=${TAG} pid=${PID} ===="
log "wait /health"
...
log "==== measure_phaseF end tag=${TAG} ===="

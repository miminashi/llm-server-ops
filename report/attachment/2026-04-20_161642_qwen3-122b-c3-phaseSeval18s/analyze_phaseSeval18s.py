#!/usr/bin/env python3
"""analyze_phaseSeval18s.py - Phase S-eval-18session 集計スクリプト

18 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval13s + phaseSeval14s + 本 Phase phaseSeval18s) を合算し、
ub 別 n=16 の session 間 σ, 18-session verdict, ピーク順序安定性、pooled 90-run 統計、
...
CUR_SESSION_LABEL = "S18_phaseSeval18s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval18s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval18s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval18s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-18session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S18) 5-run 統計（eval フェーズ）\n")
        ...
        # 5. Welch t (prior 17-session pool vs cur S18)
        f.write("\n## 5. Prior 17-session pool (S1..S17) vs 本 Phase (S18) Welch t\n")
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (18 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (18 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (18 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (18 session)\n")

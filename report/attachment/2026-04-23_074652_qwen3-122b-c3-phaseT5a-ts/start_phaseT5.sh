#!/usr/bin/env bash
# start_phaseT5.sh - Phase T-5a-ts: tensor-split (-ts) + OT pattern 起動
# T-5a-thr ベースに `-ts` (TS env var) 対応 1 行追加。TS 未指定なら従来挙動。
set -euo pipefail
...
THREADS="${THREADS:-40}"
OT_TAG="${OT_TAG:-B18}"
TS="${TS:-}"          # tensor-split, 例: "13,11,12,13" 未指定なら -ts 付与なし
...
REMOTE_LOG="/tmp/llama-server_phaseT5_${OT_TAG}_t${THREADS}_sm${SPLIT_MODE}_k${CACHE_TYPE_K}_v${CACHE_TYPE_V}_fa${FLASH_ATTN}_ctx${CTX_SIZE}_b${BATCH_SIZE}_ub${UB_SIZE}${TS_TAG}.log"
...
echo "[start_phaseT5] OT_TAG=${OT_TAG} OT_REGEX=${OT_REGEX} TS=${TS:-(default)}"
...
for i in $(seq 1 60); do
  ...
  if ssh "$HOST" "grep -qE 'ubatch.*must be|n_ubatch.*must|invalid.*ubatch|llama_init.*failed|unsupported KV|unknown cache-type|split.?mode.*not supported|invalid.*threads|threads.*must|tensor.split.*invalid|invalid.*tensor.split' ${REMOTE_LOG} 2>/dev/null"; then

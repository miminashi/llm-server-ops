#!/usr/bin/env python3
"""analyze_phaseSeval57s.py - Phase S-eval-57session 集計スクリプト

56 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval56s + 本 Phase phaseSeval57s) を合算し、
ub 別 n=57 の session 間 σ, 57-session verdict, ピーク順序安定性、pooled 285-run 統計、
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval57s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval57s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval57s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-57session verdict\n")

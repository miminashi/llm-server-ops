#!/usr/bin/env python3
"""analyze_phaseSeval4s.py - Phase S-eval-4session 集計スクリプト

4 session (phaseSeval + phaseSevalcross + phaseSeval3s + 本 Phase phaseSeval4s) を合算し、
ub 別 n=4 の session 間 σ, 4-session verdict, ピーク順序安定性、pooled 20-run 統計を出力する。
"""
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval4s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval4s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval4s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-4session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S4) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 4 session mean 時系列
        f.write("\n## 2. 4 session mean 時系列 (eval 5-run mean, t/s)\n")
        f.write("ub    | S1_Seval | S2_Sevalcross | S3_Seval3s | S4_Seval4s | range | mean_of_4 | σ_session\n")
        f.write("------|----------|---------------|------------|------------|-------|-----------|----------\n")
        ...
        for ub in UBS:
            ...
            if len(finite) >= 2:
                ...
                s1 = f"{means_per_session[0]:8.3f}" if not math.isnan(means_per_session[0]) else "   NA   "
                s2 = f"{means_per_session[1]:13.3f}" if not math.isnan(means_per_session[1]) else "      NA     "
                s3 = f"{means_per_session[2]:10.3f}" if not math.isnan(means_per_session[2]) else "    NA    "
                s4 = f"{means_per_session[3]:10.3f}" if not math.isnan(means_per_session[3]) else "    NA    "
                f.write(
                    f"{ub}  | {s1} | {s2} | {s3} | {s4} | "
        ...
        # 5. Welch t (prior 3-session pool vs cur S4)
        f.write("\n## 5. Prior 3-session pool (S1+S2+S3) vs 本 Phase (S4) Welch t\n")
        ...
        # 6. pooled 20-run
        f.write("\n## 6. Pooled 20-run (S1+S2+S3+S4) 統計\n")
        ...
        # 9. 4-session verdict summary
        f.write("\n## 9. 4-session verdict summary (range ベース)\n")
        for ub in UBS:

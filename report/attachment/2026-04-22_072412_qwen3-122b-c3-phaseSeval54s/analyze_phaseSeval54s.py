#!/usr/bin/env python3
"""analyze_phaseSeval54s.py - Phase S-eval-54session 集計スクリプト

53 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval53s + 本 Phase phaseSeval54s) を合算し、
ub 別 n=54 の session 間 σ, 54-session verdict, ピーク順序安定性、pooled 270-run 統計、
...
CUR_SESSION_LABEL = "S54_phaseSeval54s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval54s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval54s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval54s_verdict.txt"
    with verdict_path.open("w") as f:
        ...
        # 1. 本 Phase (S54) 5-run 統計
        f.write("## 1. 本 Phase (S54) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 54 session mean 時系列
        f.write("\n## 2. 54 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 53-session pool vs cur S54)
        f.write("\n## 5. Prior 53-session pool (S1..S53) vs 本 Phase (S54) Welch t\n")
        ...
        # 9. 54-session verdict summary
        f.write("\n## 9. 54-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (54 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (54 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (54 session)\n")
        ...
        # 12. ピーク順序 54-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (54 session)\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S54）\n")
        w1_1584 = None

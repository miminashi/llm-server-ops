#!/usr/bin/env python3
"""analyze_phaseSeval3s.py - Phase S-eval-3session 集計スクリプト

3 session (phaseSeval + phaseSevalcross + 本 Phase phaseSeval3s) を合算し、
ub 別 n=3 の session 間 σ, 3-session verdict, ピーク順序安定性、pooled 15-run 統計を出力する。
"""
...
CUR_SESSION_LABEL = "S3_phaseSeval3s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval3s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval3s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval3s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-3session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S3) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 3 session mean 時系列
        f.write("\n## 2. 3 session mean 時系列 (eval 5-run mean, t/s)\n")
        f.write("ub    | S1_Seval | S2_Sevalcross | S3_Seval3s | range | mean_of_means | σ_session\n")
        f.write("------|----------|---------------|------------|-------|---------------|----------\n")
        ...
        for ub in UBS:
            ...
            if len(finite) >= 2:
                ...
                f.write(
                    f"{ub}  | {s1} | {s2} | {s3} | "
                    f"{rng:5.3f} | {mm:13.3f} | {sig:8.3f}  [{vr}]\n"
                )
            else:
        ...
        # 5. Welch t (prior 2-session pool vs cur)
        f.write("\n## 5. Prior 2-session pool (S1+S2) vs 本 Phase (S3) Welch t\n")
        ...
        # 9. 3-session verdict summary
        f.write("\n## 9. 3-session verdict summary (range ベース)\n")
        for ub in UBS:

#!/usr/bin/env bash
# start_phaseSbfa0.sh - Phase Sb-fa0: 候補 K (FA workspace の ub×ctx cross 項) 検証
# Phase Sbctx からの変更点:
#   - FLASH_ATTN デフォルトを 0 に反転
#   - REMOTE_LOG プレフィックス phaseSbctx_ → phaseSbfa0_
set -euo pipefail
...
echo "[start_phaseSbfa0] FLASH_ATTN=${FLASH_ATTN} CTX_SIZE=${CTX_SIZE} -b ${BATCH_SIZE} -ub ${UB_SIZE} MAX_ITER=${MAX_ITER} (C-D3 base, poll=0, f16 KV)"
echo "[start_phaseSbfa0] remote log: ${REMOTE_LOG}"

ssh -f "$HOST" "cd ~/llama.cpp && nohup bash -c \"${LAUNCH_CMD}\" > ${REMOTE_LOG} 2>&1 < /dev/null &"

echo "[start_phaseSbfa0] waiting for /health (max ${MAX_ITER}*5s) or OOM/ub-reject abort..."
for i in $(seq 1 ${MAX_ITER}); do
  if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    echo "[start_phaseSbfa0] /health OK after ${i}*5s"
    sleep 3
    PID=$(ssh "$HOST" "ps -eo pid,comm,args | awk '\$2==\"llama-server\" {print \$1; exit}'")
    echo "[start_phaseSbfa0] PID=${PID}"
    echo "$PID"
    exit 0
  fi
  # OOM 早期判定
  if ssh "$HOST" "grep -qE 'cudaMalloc failed: out of memory|failed to allocate CUDA[0-9] buffer|graph_reserve: failed to allocate|llama_kv_cache.*failed|n_ctx.*too large|failed to allocate KV' ${REMOTE_LOG} 2>/dev/null"; then
    echo "[start_phaseSbfa0] OOM pattern detected in ${REMOTE_LOG}, abort" >&2
  ...
  if ssh "$HOST" "grep -qE 'ubatch.*must be|n_ubatch.*must|invalid.*ubatch|llama_init.*failed' ${REMOTE_LOG} 2>/dev/null"; then
    echo "[start_phaseSbfa0] -ub lower-bound rejection detected in ${REMOTE_LOG}, abort" >&2
    ssh "$HOST" "tail -30 ${REMOTE_LOG}" >&2 || true
    exit 3
  fi
  sleep 5
done
echo "[start_phaseSbfa0] FAILED to become healthy in $((MAX_ITER*5))s" >&2
ssh "$HOST" "tail -20 ${REMOTE_LOG}" >&2 || true
exit 1

#!/usr/bin/env python3
"""analyze_phaseSeval24s.py - Phase S-eval-24session 集計スクリプト

24 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval23s + 本 Phase phaseSeval24s) を合算し、
ub 別 n=24 の session 間 σ, 24-session verdict, ピーク順序安定性、pooled 120-run 統計、
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval24s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval24s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval24s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-24session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S24) 5-run 統計（eval フェーズ）\n")
        ...
        f.write("\n## 2. 24 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 23-session pool vs cur S24)
        f.write("\n## 5. Prior 23-session pool (S1..S23) vs 本 Phase (S24) Welch t\n")
        ...
        f.write("\n## 9. 24-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (24 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (24 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (24 session)\n")
        ...
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (24 session)\n")

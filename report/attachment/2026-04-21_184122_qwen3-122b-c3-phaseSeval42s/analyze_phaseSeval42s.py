#!/usr/bin/env python3
"""analyze_phaseSeval42s.py - Phase S-eval-42session 集計スクリプト

42 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval41s + 本 Phase phaseSeval42s) を合算し、
ub 別 n=42 の session 間 σ, 42-session verdict, ピーク順序安定性、pooled 210-run 統計、
...
CUR_SESSION_LABEL = "S42_phaseSeval42s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval42s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval42s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval42s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-42session verdict\n")
        ...
        # 1. 本 Phase (S42) 5-run 統計
        f.write("## 1. 本 Phase (S42) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 42 session mean 時系列
        f.write("\n## 2. 42 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 41-session pool vs cur S42)
        f.write("\n## 5. Prior 41-session pool (S1..S41) vs 本 Phase (S42) Welch t\n")
        ...
        # 9. 42-session verdict summary
        f.write("\n## 9. 42-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (42 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (42 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (42 session)\n")
        ...
        # 12. ピーク順序 42-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (42 session)\n")

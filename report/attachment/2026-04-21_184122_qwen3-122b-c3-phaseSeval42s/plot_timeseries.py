#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 42 session + Sbfine 3 reference points の時系列プロット
...
- S1..S42 (5-run mean) を x=1..42 に折れ線 + 丸 marker でプロット
...
# S1..S41 summary TSV path (phase=eval のみ抽出) + S42 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S42", None, "summary_phaseSeval42s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval42s.tsv
]
...
def main() -> int:
    ...
    ax.set_title(
        "Phase S-eval 42-session + Sbfine 3-run ref (P100 t120h-p100, ctx=32768, fa=1, OT=MoE-only, prompt_1k)"
    )

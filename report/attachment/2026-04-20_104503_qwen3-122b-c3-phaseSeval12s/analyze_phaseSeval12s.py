#!/usr/bin/env python3
"""analyze_phaseSeval12s.py - Phase S-eval-12session 集計スクリプト

12 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval11s + 本 Phase phaseSeval12s) を合算し、
ub 別 n=12 の session 間 σ, 12-session verdict, ピーク順序安定性、pooled 60-run 統計、
ub=1584/1586/1664 崩壊頻度 (Wilson 95% CI)、ub 別時系列パターン分析、
S1-S3 mode A / S4-S5 mode B / S6 mode C / S7 / S8 / S9 / S10 / S11 / S12 のモード分類比較を出力する。
"""
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval12s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval12s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval12s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-12session verdict\n")
        ...
        # 2. 12 session mean 時系列
        f.write("\n## 2. 12 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 11-session pool vs cur S12)
        f.write("\n## 5. Prior 11-session pool (S1..S11) vs 本 Phase (S12) Welch t\n")
        ...
        # 6. pooled 60-run
        f.write("\n## 6. Pooled 60-run (S1+S2+S3+S4+S5+S6+S7+S8+S9+S10+S11+S12) 統計\n")
        ...
        # 9. 12-session verdict summary
        f.write("\n## 9. 12-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (12 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (12 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (12 session)\n")
        ...
        # 12. ピーク順序 12-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (12 session)\n")
        ...
        # 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S8 D / S9 / S10 / S11 / S12) 比較
        f.write("\n## 13. モード分類 (S1-S3 A / S4-S5 B / S6 C / S8 D / S9 / S10 / S11 / S12) の eval_mean 比較\n")
        ...
        f.write("\n## 14. warmup1 ub=1584 absolute 帯のモード判定（S12）\n")
        w1_1584 = None

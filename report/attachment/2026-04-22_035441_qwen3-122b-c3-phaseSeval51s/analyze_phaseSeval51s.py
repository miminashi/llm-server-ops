#!/usr/bin/env python3
"""analyze_phaseSeval51s.py - Phase S-eval-51session 集計スクリプト

51 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval50s + 本 Phase phaseSeval51s) を合算し、
ub 別 n=51 の session 間 σ, 51-session verdict, ピーク順序安定性、pooled 255-run 統計、
...
CUR_SESSION_LABEL = "S51_phaseSeval51s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval51s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval51s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval51s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-51session verdict\n")
        ...
        f.write("## 1. 本 Phase (S51) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 51 session mean 時系列
        f.write("\n## 2. 51 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        f.write("\n## 5. Prior 50-session pool (S1..S50) vs 本 Phase (S51) Welch t\n")
        ...
        # 9. 51-session verdict summary
        f.write("\n## 9. 51-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (51 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (51 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (51 session)\n")
        ...
        # 12. ピーク順序 51-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (51 session)\n")

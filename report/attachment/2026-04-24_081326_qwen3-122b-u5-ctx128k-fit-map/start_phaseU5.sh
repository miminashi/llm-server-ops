#!/usr/bin/env bash
# start_phaseU5.sh - Phase U-5: dry-probe 用 llama-server 起動 (eval 最小化、/health 到達を主目的)
# start_phaseU1ext.sh をベースに
#   - EXTRA_ARGS を env 経由のみ (デフォルト空、spec ckpt 無し)
#   - --metrics 削除 (dry-probe では不要)
#   - --jinja 削除 (warm probe の short prompt は plain /completions)
#   - --n-predict 256 (eval 最小化)
set -euo pipefail
...
REMOTE_LOG="/tmp/llama-server_phaseU5_${OT_TAG}_t${THREADS}_sm${SPLIT_MODE}_k${CACHE_TYPE_K}_v${CACHE_TYPE_V}_fa${FLASH_ATTN}_ctx${CTX_SIZE}_b${BATCH_SIZE}_ub${UB_SIZE}${TS_TAG}${EXTRA_TAG_PART}.log"
...
echo "[start_phaseU5] OT_TAG=${OT_TAG} OT_REGEX=${OT_REGEX} TS=${TS:-(default)} EXTRA_TAG=${EXTRA_TAG}"
echo "[start_phaseU5] EXTRA_ARGS=${EXTRA_ARGS:-(none)}"
echo "[start_phaseU5] THREADS=${THREADS} CACHE_TYPE=${CACHE_TYPE_K} FA=${FLASH_ATTN} CTX=${CTX_SIZE} -b ${BATCH_SIZE} -ub ${UB_SIZE}"
echo "[start_phaseU5] remote log: ${REMOTE_LOG}"

ssh -f "$HOST" "cd ~/llama.cpp && nohup bash -c \"${LAUNCH_CMD}\" > ${REMOTE_LOG} 2>&1 < /dev/null &"

echo "[start_phaseU5] waiting for /health (max 300s) or OOM/param reject..."
START_EPOCH=$(date +%s)
for i in $(seq 1 60); do
  if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
    ELAPSED=$(( $(date +%s) - START_EPOCH ))
    echo "[start_phaseU5] /health OK after ${i}*5s (elapsed=${ELAPSED}s)"
    sleep 3
    PID=$(ssh "$HOST" "ps -eo pid,comm,args | awk '\$2==\"llama-server\" {print \$1; exit}'")
    echo "[start_phaseU5] PID=${PID}"
  ...
  if ssh "$HOST" "grep -qE 'cudaMalloc failed: out of memory|failed to allocate CUDA[0-9] buffer|graph_reserve: failed to allocate|llama_kv_cache.*failed|n_ctx.*too large|failed to allocate KV|CUDA error: out of memory|ggml_abort.*cuda' ${REMOTE_LOG} 2>/dev/null"; then
    echo "[start_phaseU5] OOM pattern detected, abort" >&2
  ...
  if ssh "$HOST" "grep -qE 'ubatch.*must be|n_ubatch.*must|invalid.*ubatch|llama_init.*failed|unsupported KV|unknown cache-type|split.?mode.*not supported|invalid.*threads|threads.*must|tensor.split.*invalid|invalid.*tensor.split|error: invalid argument|unknown argument' ${REMOTE_LOG} 2>/dev/null"; then
    echo "[start_phaseU5] param rejection detected, abort" >&2
    ssh "$HOST" "tail -40 ${REMOTE_LOG}" >&2 || true
    exit 3
  fi
  sleep 5
done
echo "[start_phaseU5] FAILED to become healthy in 300s (TIMEOUT)" >&2
ssh "$HOST" "tail -30 ${REMOTE_LOG}" >&2 || true
exit 1

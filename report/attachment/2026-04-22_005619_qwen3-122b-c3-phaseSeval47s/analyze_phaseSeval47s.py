#!/usr/bin/env python3
"""analyze_phaseSeval47s.py - Phase S-eval-47session 集計スクリプト

47 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval46s + 本 Phase phaseSeval47s) を合算し、
ub 別 n=47 の session 間 σ, 47-session verdict, ピーク順序安定性、pooled 235-run 統計、
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval47s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval47s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval47s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-47session verdict\n")
        ...
        # 1. 本 Phase (S47) 5-run 統計
        f.write("## 1. 本 Phase (S47) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 47 session mean 時系列
        f.write("\n## 2. 47 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 46-session pool vs cur S47)
        f.write("\n## 5. Prior 46-session pool (S1..S46) vs 本 Phase (S47) Welch t\n")
        ...
        # 9. 47-session verdict summary
        f.write("\n## 9. 47-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (47 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (47 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (47 session)\n")
        ...
        # 12. ピーク順序 47-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (47 session)\n")

#!/usr/bin/env python3
"""MTP on/off comparison figure for llama-split-bench runs on t120h-p100 (P100 x4).

Adapted from report/attachment/2026-09-16_103619_aws_gpu01_split_mode_mtp_3arm/plot_mtp2.py:
only labels (7-way -> 4-way), the machine line and the AllReduce note differ. Here the
tensor arm runs on the default NCCL path in BOTH runs (no GGML_CUDA_ALLREDUCE=none).

Original notes (aws-gpu01 3-arm edition):
...
Encoding (composite, so color never carries MTP state alone):
  color     = split configuration   (layer 4-way / tensor 4-way / layer 2-way)
...
NAME_JA = {"layer": "layer 4枚", "tensor": "tensor 4枚", "layer2": "layer 2枚"}
NAME_EN = {"layer": "layer 4-way", "tensor": "tensor 4-way", "layer2": "layer 2-way"}
...
def main():
    ...
    ax.set_ylim(0, ax.get_ylim()[1] * 1.45)   # headroom so the legend sits above the lines
    ax.legend(fontsize=8.2, frameon=False, labelcolor=MUTED, ncol=3, loc="upper right")
    ...
    tag = lambda d: json.load(open(os.path.join(d, "run-info.json"))).get("tag", os.path.basename(d))
    if ja:
        sup = (f"t120h-p100 — Tesla P100-PCIE-16GB ×4 / Qwen3.8-27B UD-Q4_K_XL / KV q8_0 / -fa on / ctx={info['ctx']:,}"
               f"\nMTP off = {tag(a.off)} ／ MTP on = {tag(a.on)}"
               f"（--spec-type draft-mtp --spec-draft-n-max 2）。"
               f"全腕とも既定の NCCL AllReduce 経路")
    else:
        sup = (f"t120h-p100 - Tesla P100-PCIE-16GB x4 / Qwen3.8-27B UD-Q4_K_XL / KV q8_0 / -fa on / ctx={info['ctx']:,}"
               f"\nMTP off = {tag(a.off)} / MTP on = {tag(a.on)}"
               f" (--spec-type draft-mtp --spec-draft-n-max 2). "
               f"all arms on the default NCCL AllReduce path")

#!/bin/bash
# tensor 分割 + MTP の起動プローブ。$1 = 試行回数, $2 = GGML_CUDA_ALLREDUCE の値 (空なら既定)
M=$HOME/models/Qwen3.8-27B-GGUF/Qwen3.8-27B-UD-Q4_K_XL.gguf
N=${1:-3}; AR=${2:-}
...
for i in $(seq 1 $N); do
  ...
  setsid nohup $PREFIX ~/llama.cpp/build/bin/llama-server -m "$M" --host 127.0.0.1 --port 18099 \
    ...
    --parallel 1 -t 16 --jinja --cache-type-k q8_0 --cache-type-v q8_0 --spec-type draft-mtp --spec-draft-n-max 2 > $LOG 2>&1 < /dev/null &
  R=""

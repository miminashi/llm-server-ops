#!/usr/bin/env python3
"""analyze_phaseSeval16s.py - Phase S-eval-16session 集計スクリプト

16 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval13s + phaseSeval14s + 本 Phase phaseSeval16s) を合算し、
ub 別 n=16 の session 間 σ, 16-session verdict, ピーク順序安定性、pooled 80-run 統計、
...
CUR_SESSION_LABEL = "S16_phaseSeval16s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval16s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval16s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval16s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-16session verdict\n")
        ...
        # 1. 本 Phase 5-run 統計
        f.write("## 1. 本 Phase (S16) 5-run 統計（eval フェーズ）\n")
        ...
        # 5. Welch t (prior 15-session pool vs cur S15)
        f.write("\n## 5. Prior 15-session pool (S1..S16) vs 本 Phase (S16) Welch t\n")
        ...
        # 9. 16-session verdict summary
        f.write("\n## 9. 16-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (16 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (16 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (16 session)\n")
        ...
        # 12. ピーク順序 16-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (16 session)\n")

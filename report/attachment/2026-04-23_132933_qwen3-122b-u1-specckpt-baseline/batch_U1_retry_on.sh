#!/usr/bin/env bash
# batch_U1_retry_on.sh - Phase U-1: ON 条件のみ再測定
# 初回 (ctx-checkpoints 4, -cpent default 8192) は CUDA OOM で全滅したため、
# ctx-checkpoints 1 + -cpent -1 (prefill 中の ckpt 作成無効) に変更して VRAM 使用を最小化。
set -euo pipefail
...
# 初回 ctx-checkpoints 4 で CUDA OOM (device 3, 149.063 MiB/ckpt × 4)
# 改善: ctx-checkpoints 1 (最小) + -cpent -1 (prefill 中の auto-ckpt 生成無効)
SPEC_ON_ARGS="${SPEC_ON_ARGS:---spec-type ngram-mod --ctx-checkpoints 1 --checkpoint-every-n-tokens -1 --spec-ngram-size-n 24 --draft-min 48 --draft-max 64}"

CONDITIONS=(
  "ON_prompt1k_retry#ON#prompt_1k.txt"
  "ON_code_retry#ON#prompt_code.txt"
  "ON_repetitive_retry#ON#prompt_repetitive.txt"
)

SKIP_LABELS="${SKIP_LABELS:-}"

echo "[batchU1-retry] start at $(TZ=Asia/Tokyo date +'%Y-%m-%d %H:%M:%S')"
echo "[batchU1-retry] SPEC_ON_ARGS=${SPEC_ON_ARGS}"
...
for COND in "${CONDITIONS[@]}"; do
  ...
  if [[ ",${SKIP_LABELS}," == *",${LABEL},"* ]]; then
    echo "[batchU1-retry] SKIP label=${LABEL}"
    continue
  fi

  EXTRA_ARGS="$SPEC_ON_ARGS"
  EXTRA_TAG="specONretry"
  TAG_COND="U1_${OT_TAG}_${LABEL}_t${THR}_kv${KV}_sm${SM}_ctx${CTX}_ub${UB}"

  echo "[batchU1-retry] ================================"
  echo "[batchU1-retry] cond: label=${LABEL} mode=${MODE} prompt=${PROMPT_BN} at $(TZ=Asia/Tokyo date +'%H:%M:%S')"
  echo "[batchU1-retry] ================================"
  ...
  for i in $(seq 1 60); do
    if curl -sf -m 5 http://10.1.4.14:8000/health > /dev/null 2>&1; then
      echo "[batchU1-retry] /health OK after ${i}*5s"
      healthy=1
      break
    fi
    if ! kill -0 "$START_PID" 2>/dev/null; then
      echo "[batchU1-retry] start exited early for label=${LABEL}" >&2
      break
    fi
    sleep 5
  done
  ...
  if [ "$healthy" -ne 1 ]; then
    echo "[batchU1-retry] ERROR: health never OK for label=${LABEL}" >&2
  ...
  if [ -z "$PID" ]; then
    echo "[batchU1-retry] ERROR: PID not found for label=${LABEL}" >&2
    continue
  fi
  echo "[batchU1-retry] PID=$PID"
  ...
  echo "[batchU1-retry] startup log saved ($(wc -l < startup_logs/${TAG_COND}.log) lines)"
  ...
  echo "[batchU1-retry] measure done ($(tail -1 run_${TAG_COND}.log))"

  bash "$SKILL_STOP" "$HOST" > /dev/null 2>&1 || true
  sleep 5
done

echo "[batchU1-retry] end at $(TZ=Asia/Tokyo date +'%Y-%m-%d %H:%M:%S')"

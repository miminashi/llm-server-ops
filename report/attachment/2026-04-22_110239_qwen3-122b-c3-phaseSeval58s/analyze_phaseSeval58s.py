#!/usr/bin/env python3
"""analyze_phaseSeval58s.py - Phase S-eval-58session 集計スクリプト

57 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval57s + 本 Phase phaseSeval58s) を合算し、
ub 別 n=58 の session 間 σ, 58-session verdict, ピーク順序安定性、pooled 290-run 統計、
...
CUR_SESSION_LABEL = "S58_phaseSeval58s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval58s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval58s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval58s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-58session verdict\n")

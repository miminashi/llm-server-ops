#!/usr/bin/env python3
"""plot_timeseries.py - Phase S-eval 58 session + Sbfine 3 reference points の時系列プロット
...
- S1..S58 (5-run mean) を x=1..58 に折れ線 + 丸 marker でプロット
- 崩壊閾値 eval_mean=15.0 を水平線で表示
- S1..S58 に対する線形回帰直線（trend line）を各 ub ごとに dashed で重ねる
"""
...
# S1..S58 summary TSV path (phase=eval のみ抽出) + S58 (本 Phase)
S_EVAL_DIRS = [
    ...
    ("S58", None, "summary_phaseSeval58s.tsv"),  # 本 Phase、SCRIPT_DIR/summary_phaseSeval58s.tsv
]
...
def main() -> int:
    ...
    for ub in UBS:
        ...
        # 回帰直線 (S1..S58 only、Sbfine ref は除外)

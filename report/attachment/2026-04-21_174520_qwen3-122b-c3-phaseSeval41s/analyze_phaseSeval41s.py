#!/usr/bin/env python3
"""analyze_phaseSeval41s.py - Phase S-eval-41session 集計スクリプト

41 session (phaseSeval + phaseSevalcross + phaseSeval3s ... phaseSeval40s + 本 Phase phaseSeval41s) を合算し、
ub 別 n=41 の session 間 σ, 41-session verdict, ピーク順序安定性、pooled 205-run 統計、
...
CUR_SESSION_LABEL = "S41_phaseSeval41s"
...
def main() -> int:
    ...
    # run 別 raw TSV
    summary_path = SCRIPT_DIR / "summary_phaseSeval41s.tsv"
    ...
    # 統計 CSV
    stats_path = SCRIPT_DIR / "phaseSeval41s_stats.csv"
    ...
    # verdict
    verdict_path = SCRIPT_DIR / "phaseSeval41s_verdict.txt"
    with verdict_path.open("w") as f:
        f.write("# Phase S-eval-41session verdict\n")
        ...
        # 1. 本 Phase (S41) 5-run 統計
        f.write("## 1. 本 Phase (S41) 5-run 統計（eval フェーズ）\n")
        ...
        # 2. 41 session mean 時系列
        f.write("\n## 2. 41 session mean 時系列 (eval 5-run mean, t/s)\n")
        ...
        # 5. Welch t (prior 40-session pool vs cur S41)
        f.write("\n## 5. Prior 40-session pool (S1..S40) vs 本 Phase (S41) Welch t\n")
        ...
        # 9. 41-session verdict summary
        f.write("\n## 9. 41-session verdict summary (range ベース)\n")
        for ub in UBS:
        ...
        f.write("\n## 11. ub=1664 時系列パターン分析 (41 session)\n")
        ...
        f.write("\n## 11b. ub=1586 時系列パターン分析 (41 session)\n")
        ...
        f.write("\n## 11c. ub=1584 時系列パターン分析 (41 session)\n")
        ...
        # 12. ピーク順序 41-session 集計
        f.write("\n## 12. ピーク 1 位 ub の出現頻度 (41 session)\n")

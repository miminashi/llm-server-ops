# aws-gpu01/02 のファン静音化 — 起動時も含め 6,500→2,900rpm

- **実施日時**: 2026年8月17日 21:45 〜 8月18日 17:30 JST (BMC ファン制御の実測、温度連動デーモンの実装と両機常設、負荷試験、BIOS 設定変更、gpu01 の起動不能からの復旧。8月18日 02:00〜02:10 に追試＝boot-quiet の自動併走化と fan mode 書き直しの修正、02:38〜02:50 に再起動不要な残課題の追跡＝gpu02 の DIMM 実装状況と gpu01 のスロット対応の確定、17:23〜17:30 にユーザ立ち会いでの電源 OFF/ON ＝ 修正後 boot-quiet の最終測定)
- **報告日時**: 2026年8月18日 14:15 JST (初版 8月17日 23:44、その後 追試・未記載事項・残課題の追跡結果を追記)
- **作成者**: Claude Opus 5
...
次に本題の「起動時の爆音」に取り組んだ。起動処理中も外部から回転数を指定できるものの、当初はこちらの指定が何度も打ち消され、完全には抑えられなかった。原因を追ううちに、打ち消していたのは相手ではなく自分の実装だと分かった。回転数を指定するたびに動作モードまで書き直しており、そのモード切り替えのたびに全ファンが最大回転に戻される仕様だったためである。モードは必要なときだけ設定するよう直したうえで、最後にユーザ立ち会いで電源を落として入れ直したところ、**起動中も通常運転と同じ静かな回転数のまま通った**。起動が完了してからは、デーモンが十数秒で制御を引き継ぐ。
...
作業中に本題以外の問題も 2 つ見つかった。ひとつは aws-gpu02 の起動画面に特定のメモリモジュールの不良が報告されていること、もうひとつは 2 台を繋ぐ 100GbE の設定が永続化されておらず、再起動すると失われる状態だったことである。後者はその場で永続化して再起動で検証まで済ませた。前者はセッションの最後に追跡したところ、報告されているスロットには現在そもそもメモリが装着されておらず、過去の故障情報が起動画面に残り続けているだけと分かった。現時点で実害はないが、その機体は片方の CPU 側にメモリが 1 枚も無い構成であることも同時に判明したので、こちらは増設を検討する材料として記録した。
...
起動時の爆音は、最後の確認で通常運転と同じ静かさまで抑えられた。ただし抑制が働かない状況（抑制を明示的に止めた場合や、作業マシンからサーバの管理系に届かない場合）では従来どおり全開で回るため、電源操作のガードは念のため維持する。起動しなくなった機体については、どの拡張スロットに何が載っているかを OS 側から調べ直し、次に手を入れるときはグラフィックスカードの載ったスロットだけを対象にすればよい、というところまで詰めた。実際の適用には再起動が要るので、次に再起動する機会に回す。
...
- 測定ログ（CSV）
  ...
  - **最終測定（修正後の実装でコールドブート）**: [gpu01](attachment/2026-08-17_234403_aws_gpu_fan_noise_reduction/boot_gpu01_final.csv) / [gpu02](attachment/2026-08-17_234403_aws_gpu_fan_noise_reduction/boot_gpu02_final.csv)
...
BMC は POST 中も IPMI を受け付ける。当初の測定では投入した duty が何度も 100% に書き戻され、
**「POST 中の BMC はファン制御を手放さない」と結論した**が、**これは誤りだった** —
後述の最終測定で、原因は `boot-quiet.sh` 自身が毎周期 fan mode を書き直して
BMC の 100% リセットを誘発していたことだと判明している（「追試」と「最終測定」を参照）。
以下はその当初実装での測定値。
...
**上表の POST 中の測定値（中央値 3,636rpm 等）はいずれも当初実装で採ったもの**である。

#### 最終測定: 修正後の実装でコールドブート（2026年8月18日 17:23〜17:27 JST）

ユーザが実機の前で起動音を確認するため、**両機を ACPI シャットダウンして電源を入れ直した**
（`bmc-power.sh soft` → 電源断を確認 → `on`。boot-quiet は `bmc-power.sh` から自動併走）。

| 機体 | POST 中 FAN1-8 平均の中央値 | レンジ | サンプル数 |
|---|---|---|---|
| aws-gpu01 | **2,925 rpm** | 2,925–3,750 | 5 |
| aws-gpu02 | **2,914 rpm** | 2,914–2,929 | 4 |

**起動中も定常運転と同じ 2,900rpm 台で通せた**（当初実装の 3,636rpm、対策前相当の 11,000rpm 台と対比）。
つまり **POST 中の爆音は事実上解消した**。fan mode の読み値には `02`(Optimal) が混じっており
**BMC が POST 中に mode を Optimal へ戻すことはある**が、boot-quiet が 10 秒以内に Full へ戻して
duty を書き直すため、影響は gpu01 で一瞬 3,750rpm に触れた程度に留まっている。

**当初の結論「POST 中は BMC がファン制御を手放さないので競り負ける」は誤りだった。**
正しくは「**BMC は POST 中も duty 指定を受け付けるが、fan mode を書き直すと全 zone が 100% に
リセットされる**」であり、mode を書き直さなければ低 duty を維持できる。

**ユーザの体感（実機前で確認）**: 「ファンは数秒だけ唸っていたが、だいぶ抑制できている」。
残った「数秒の唸り」は、電源投入直後（boot-quiet が最初の duty を書き込むまでの BMC 初期化中）と、
BMC が一瞬 mode を Optimal に戻した際の 100% リセットに対応すると考えられる
（記録上も gpu01 で一時 3,750rpm に触れている）。

電源投入からの所要時間（コールドブート、`on` 発行を起点）:
...
gpu01 が 65 秒長いが、**BIOS 設定の差（スロット OPROM）とハードウェア差（GPU 7 枚 vs 6 枚、
RAM 160GB vs 96GB）が混在**しており、OPROM 単独の寄与は分離できていない。
ただし gpu01 でも GPU スロットの OPROM を切れば短縮が見込める傍証にはなる。
...
1. **aws-gpu02 の POST に DIMM 不良の報告 — 該当スロットは空だった**:
   `Failing DIMM:DIMM location(Uncorrectable memory component found) / P2-DIMME1`
   （[スクリーンショット](attachment/2026-08-17_234403_aws_gpu_fan_noise_reduction/gpu02_post_failing_dimm.png)）。
   セッション終了前に追跡したところ、**aws-gpu02 に実装されている DIMM は `P1_DIMMA1/A2/A3` の
   32GB × 3 枚（96GB）だけで、報告されている `P2_DIMME1` を含む他 11 スロットはすべて
   "No Module Installed"** だった。EDAC の CE/UE カウンタも全て 0、SEL にメモリ関連イベントなし。
   つまり **BIOS が過去の故障情報を保持して POST に表示し続けている**状態で、実害はない。
   比較のため aws-gpu01 を見ると `P1_DIMMA1/A2/A3` + **`P2_DIMME1`/`P2_DIMME2`** の 32GB × 5 枚
   （160GB）で、**同じ `P2_DIMME1` に正常な DIMM が載っている**。gpu02 のそれは
   故障して抜かれたと考えるのが自然
2. **aws-gpu02 は CPU2 側にメモリが 1 枚も無い（NUMA が片寄っている）**: 上記の帰結。
   `numactl --hardware` で **node1 size = 0 MB**（gpu01 は node0 96,540MB / node1 64,500MB と
   両ノードに存在）。aws-gpu02 の GPU のうち `84/88/89:00.0` の 3 枚は CPU2 側にぶら下がるため、
   **ホスト↔デバイス転送が QPI 経由になる**。既存の RPC 分散の性能実測はすべてこの構成で
   得た値であり、DIMM を増設すれば改善の余地がある（未検証）
3. **gpu01 のスロット ↔ デバイス対応が判明**（次回の BIOS 作業用。`dmidecode -t 9` と `lspci` の突合）:

   | BIOS のスロット項目 | BDF | 実際のデバイス |
   |---|---|---|
   ...
   | （SMBIOS のスロット一覧に無い） | **82:00.0** | **LSI MegaRAID SAS-3 3108＝ブートディスク** |

   **SAS HBA は SMBIOS のスロット一覧に現れない**（内部 AOC 扱い）ため、BIOS のどのスロット項目に
   対応するかは特定できなかった。したがって次回は「**GPU が載っている 7 スロット
   (SLOT3,4,5,9,10,11,12) だけを Disabled にする**」のが安全な方針になる
   （VGA OpROM の実行を省けて POST 短縮の効果は得られ、SAS HBA と NIC の OpROM は残る）

   > **【2026-08-18 追記・訂正】** 続くセッションの調査で以下が判明した。詳細は
   > [2026-08-18_185344_aws_gpu01_sas_hba_slot_id.md](./2026-08-18_185344_aws_gpu01_sas_hba_slot_id.md)。
   >
   > - **SAS HBA は BIOS の `CPU2 Slot6 PCI-E x16 OPROM` が制御している**。上表で「空き」と
   >   していた `CPU2 SLOT6` がそれにあたる。SMBIOS Type 9 は当該スロットを `In Use` と報告
   >   しつつ Bus Address を `0000:01:00.0` と書くが、そこは実際には空 (`00:01.0` は
   >   `LnkSta Width x0`) で、**BIOS の誤報告**だった。HBA は CPU2 のルートポート `80:03.0`
   >   (ACPI `\_SB.PCI1.QR3A`) 配下にあり、**CPU2 側の拡張カードは HBA 1 枚だけ**である
   > - **gpu01 は Legacy BIOS (CSM) ブート**（`/sys/firmware/efi` 無し、`efibootmgr` も空）。
   >   よって MegaRAID の **Legacy Option ROM が無いとブートできない**。一方 **gpu02 は
   >   UEFI ブート**（`sda1` が `/boot/efi`）+ オンボード SATA なので同じ変更で無事だった。
   >   これが本節の事故の完全な説明になる
   > - **上表の「BIOS のスロット項目」は SMBIOS `Designation` と 1:1 対応する**（BIOS 実画面で
   >   確認）。ただし**本レポートに添付した BIOS スクリーンショットは gpu02 のもの**（11 項目）で、
   >   gpu01 は 12 項目である
   > - **`CPU2 Slot6` 以外の 11 項目を Disabled にしても正常起動する**ことを実機で確認済み。
   >   ただし **POST 短縮効果は reset 起点の実測で 146 秒 → 144 秒 (−2 秒) しかなく**、
   >   GPU の VGA OpROM は POST 時間の主因ではなかった
4. **ロックはサーバの `/tmp/gpu-server-locks/` にあるので再起動で消える**: `lock.sh` は対象サーバ上に
   シンボリックリンクを作る実装なので、**再起動を伴う作業をすると自分のロックが自然消滅する**。
   実際に今回、両機の再起動後にロックが `available` に戻っていた（ジョブは稼働中のまま）。
   再起動を伴う作業の後は **ロックを取り直す**こと（本セッション終了時に取り直し済み）
5. **100GbE (192.168.100.x) が永続化されていなかった**: netplan（`50-cloud-init.yaml`）には管理系のみ定義されており、RPC 用の 100GbE は手動設定だった。gpu02 の再起動で IP・リンクとも消失。新規ファイル `60-rpc-100gbe.yaml` を両機に置いて永続化し、**gpu01 の再起動で自動復帰（192.168.100.1/24、MTU 9000、100000Mb/s）を確認**
6. **BMC の Optimal は負荷時に duty 68–70% まで上げる**: 「起動後もまあまあうるさい」の主因はこれ。静音化の効果は起動時よりむしろ定常運転側に大きかった
7. **gpu02 の FAN7 は引き続き `No Reading`**（登録時から既知）。ファン fail 検知による全開化は起きていない

## 残課題

- ~~**gpu01 の Option ROM 再挑戦**~~ → **2026-08-18 に実施・完了**（[2026-08-18_185344_aws_gpu01_sas_hba_slot_id.md](./2026-08-18_185344_aws_gpu01_sas_hba_slot_id.md)）。
  **触ってはいけないのは `CPU2 Slot6`（= SAS HBA）**で、他の 11 項目は Disabled にしても起動する。
  ただし **POST 短縮効果は −2 秒しか無く、当初の目的は達成できなかった**
- ~~POST 時間の before/after 測定~~ → **完了**。コールドブート起点（gpu01 150 秒 / gpu02 85 秒）に加え、
  2026-08-18 に **reset 起点で変更前後を揃えて実測**した（146 秒 → 144 秒）。「11. POST 時間の短縮効果 —
  定量化できていない」はこれで解消（[2026-08-18_185344_aws_gpu01_sas_hba_slot_id.md](./2026-08-18_185344_aws_gpu01_sas_hba_slot_id.md)）
- ~~修正後の `boot-quiet.sh` で POST 中の回転を測り直す~~ → **完了**（2,914–2,925rpm、上記 7 の最終測定）
- ~~aws-gpu02 の P2-DIMME1 の追跡~~ → **解決**（副次発見 1・2）。該当スロットは空で EDAC エラーも 0。
  残るのは「CPU2 側に DIMM を増設するか」という運用判断（NUMA の片寄り解消、性能影響は未検証）

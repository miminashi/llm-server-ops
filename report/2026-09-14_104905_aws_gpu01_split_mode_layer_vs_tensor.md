...
> `GGML_CUDA_ALLREDUCE` を併せて設定すること。詳細は[原因調査レポート](./2026-09-14_142115_aws_gpu01_tensor_split_nccl_hang.md)。
>
> **【2026-09-16 11:05 JST 追記】上の `butterfly` は不正値で、正しくは `none` である**（挙動は同一）。
> **さらに重要な点として、その回避策には本レポートの tensor 腕の長所を打ち消す代償があることが判明した。**
> 条件をそろえて測ると `none` は NCCL に対し **decode -25%（18.34→13.81 t/s）、prefill -78〜-80%（pp512 で 384.4→83.8 t/s）**、
> GPU 利用率も 75.6% → 48.7%。**本レポートが強調した「pp512 で layer の 3 倍」という優位は回避策を入れると失われ、layer にも劣る。**
> なお本レポートの tensor 腕（NCCL 経路）は**同一 argv で 10 回試しても再現できなかった**（原因調査レポートの 0/3 と通算 0/13）ため、
> 本レポートの値は依然として**起動に成功した 1 回**に依存している。詳細は[3 腕再測定レポート](./2026-09-16_103619_aws_gpu01_split_mode_mtp_3arm.md)。
>
> **【2026-09-16 18:50 JST 追記】本レポートの「layer 分割は枚数を増やしても decode が改善しない」は、2 枚と 7 枚の 2 点しか根拠が無かったが、
> 2→7 枚を 1 枚刻みで埋めるスイープを実施し、6 点すべてで成立することを確認した**（2 枚 12.12 → 7 枚 11.37 t/s、全深度で -5〜-7%）。
> 本レポートの `layer` / `layer2` 腕の値は**小数点以下まで再現**している。
> **同時に、`--split-mode tensor` には最適枚数があり、それは 7 枚ではなく 3〜4 枚であることが判明した**
> （decode は深部で 4 枚 16.97 対 7 枚 13.39 t/s、prefill は 5→6 枚で -38% の崖）。
> また layer の prefill には **2 枚と 3 枚が同値**という未報告の段差がある。
> 詳細は[枚数スイープレポート](./2026-09-16_182920_aws_gpu01_split_mode_gpu_count_sweep.md)。
...
- [P100 4 枚でも dense 27B なら tensor 分割が全面的に勝つ](./2026-09-18_131856_t120h_p100_split_mode_qwen38_27b.md) — 本レポートと同じ Qwen3.8-27B を t120h-p100 (P100×4) で測った続編。**tensor が全深度・全指標で勝ち（decode +128〜+185%、prefill +37〜+42%）、本レポートの結論は dense モデルでは機種・枚数を変えても成り立つ**。layer 2 枚の値は本レポートと -1% 以内で一致

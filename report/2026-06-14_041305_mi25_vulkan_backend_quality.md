...
- **MI25 4枚目のランタイム脱落**: 本計測時、ROCm/Vulkan とも実コンピュートは3枚のみ（rocm-smi は4枚列挙）。`llvmpipe` が Vulkan デバイス3に出現。原因未特定（dmesg は sudo 必要で未確認）。本番運用前に電源リセット等で4枚復旧の確認が望ましい。
  - **【follow-up 2026-06-14 で原因確定】** [mi25 4枚目MI25脱落の原因究明レポート](2026-06-14_131713_mi25_gpu4_pcie_dropout.md) を参照。原因は **PCIe 物理層障害（リンク学習失敗・PresDet 陰性）** で、コールド電源サイクル13回・BIOS 精査でも遠隔復旧は不可能と確定。BIOS は正常（Above-4G=Enabled, MMIO High=512GB, 全GPUスロット Gen3/x16）。スロット SLOT4（`00:03`）が支配的に脱落、SLOT8（`80:02`）が x8 で限界。**物理的な再装着・ライザー/補助電源点検が必要**。なお本検証の「rocm-smi 4枚列挙」はカーネル証拠（amdgpu 3枚のみ初期化）と矛盾し誤観測だった。

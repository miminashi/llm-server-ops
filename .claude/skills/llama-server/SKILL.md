...
|--------|---------------------|-------------|
| Qwen3.5-35B-A3B | なし（thinking有効） | `--temp 0.6 --top-p 0.95 --top-k 20 --min-p 0 --presence-penalty 1.0 --dry-multiplier 0.8` |
...
| Qwen3.5-122B-A10B | なし（thinking有効） | `--temp 0.6 --top-p 0.95 --top-k 20 --min-p 0 --presence-penalty 1.0 --dry-multiplier 0.8` |
| Qwen3.6-27B / 27B-MTP | なし（thinking有効） | `--temp 0.6 --top-p 0.95 --top-k 20 --min-p 0 --presence-penalty 1.0 --dry-multiplier 0.8` |
| Qwen3.6-35B-A3B / 35B-A3B-MTP | なし（thinking有効） | `--temp 0.6 --top-p 0.95 --top-k 20 --min-p 0 --presence-penalty 1.0 --dry-multiplier 0.8` |
...
**反復対策パラメータについて**: Qwen3.x thinking モードは opencode 等の長コンテキストで段落単位の verbatim ループに陥ることがあるため、Qwen 公式推奨の `presence_penalty` (0〜2 のうち中庸の 1.0) と llama.cpp の DRY サンプラ (`--dry-multiplier 0.8`、base=1.75、allowed-length=2、breakers `\n : " *` は default) をデフォルト有効化している。クライアント側で値を送れば上書き可能。

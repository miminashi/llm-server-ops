...
ロックファイルはGPUサーバ上（`/tmp/gpu-server-locks/`）に保存され、SSH経由で操作します。これにより、**異なるマシンで動作する複数のClaudeセッション**が同じGPUサーバへのアクセスを安全に調整できます。
...
シンボリックリンクのアトミック性を利用したロック機構です。ロック操作はSSH経由でGPUサーバ上で実行されます：

- **ロックファイル**: GPUサーバ上の `/tmp/gpu-server-locks/<server>.lock`
- **アトミック性**: `ln -s` は既存ファイルがあると失敗するため、複数プロセスが同時にロックを取得しようとしても1つのみ成功
- **ロック情報**: シンボリックリンクの参照先にセッションIDを格納
- **セッションID**: `hostname-pid-timestamp` 形式（発信元ホスト名を含むため、どのマシンがロックを保持しているか識別可能）
...
1. **ロック保持者を確認**: `lock-status.sh` でセッションIDを確認（ホスト名から発信元マシンを特定可能）
...
- **GPUサーバ再起動時**: `/tmp` のファイルはクリアされるため、ロックも解放されます。
  **再起動を伴う作業の後は、ジョブを動かし続けるならロックを取り直すこと**（2026-08-17 の
  aws-gpu01/02 のファン静音化作業で、両機を再起動した結果 llama-server が稼働したまま
  ロックだけが `available` に戻っていた）
- **GPUサーバ電源OFF時**: SSH接続に失敗するため、`lock-status.sh` では `UNREACHABLE` と表示されます（5秒タイムアウト）

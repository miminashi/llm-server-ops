...
- **2 台セットで 1 つの llama-server を動かす RPC 分散がデフォルト構成**（2026-08-18 制定）。
  aws-gpu01 がメインホスト、aws-gpu02 が RPC ワーカーで、API は `http://10.8.2.1:8000/v1` のみ。
  起動は `.claude/skills/llama-server/scripts/llama-up.sh aws-gpu01`。**どちらを使う場合も
  両機のロックを取ること**。詳細は
  [llama-server SKILL.md](../llama-server/SKILL.md) の「RPC 分散構成」節
...
4. **aws-gpu01 / aws-gpu02 は 2 台セットで選ぶ**: RPC 分散がデフォルト構成なので、
   どちらか一方だけを選ぶことはしない（**ロックも両機に対して取る**）。200 GiB の VRAM を
...
## BMC緊急操作（IPMI / KVMスクリーンショット）

**OSがハング/クラッシュしてSSHが効かない**ときの out-of-band 復旧手段。
Supermicro機（mi25）は Redfish が DCMS ライセンス未活性で使えないため **IPMI（ipmitool）** を用いる。
詳細は [bmc.md](bmc.md) を参照。

```bash
# 電源状態 / ハードリセット（OSハングからの復旧本命）
.claude/skills/gpu-server/scripts/bmc-power.sh mi25 status
.claude/skills/gpu-server/scripts/bmc-power.sh mi25 reset

# KVMスクリーンショット（POST/パニック/BIOS画面を画像化）
.claude/skills/gpu-server/scripts/bmc-screenshot.sh mi25 /tmp/mi25.png
```

初回は venv 構築と認証情報登録が必要：
...
| サーバ | BMC/iLOホスト | トランスポート | 電源スクリプト |
|--------|--------------|----------------|----------------|
